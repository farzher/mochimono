...
if (pane) {
  ...
  style.textContent = `
    ...
    #storagePane.storage-v3-minimal .storage-v3-storage-check .storage-v2-ring{width:100px!important;height:100px!important;border:1px solid #344b3a!important;border-radius:28px!important;background:#18231b!important;box-shadow:none!important}
    ...
    #storagePane.storage-v3-minimal #showFolderAdd,#storagePane.storage-v3-minimal #showBackupAdd{display:none!important}
    .storage-v3-add-slot{width:calc(50% - 7px);min-height:132px;display:grid;place-items:center;gap:9px;margin-top:14px;padding:20px;border:1px dashed #39343a;border-radius:20px;background:#100f11;color:#716a68;cursor:pointer;transition:background .14s ease,border-color .14s ease,color .14s ease,transform .14s ease}
    .storage-v3-add-slot:hover,.storage-v3-add-slot:focus-visible{background:#161417;border-color:#5a5057;color:#b8afab;transform:translateY(-1px);outline:none}
    .storage-v3-add-slot svg{width:44px;height:44px;fill:none;stroke:currentColor;stroke-width:1.25;stroke-linecap:round;stroke-linejoin:round;opacity:.8}
    .storage-v3-add-slot strong{font-size:11px;font-weight:680;color:inherit;letter-spacing:-.01em}
    .storage-v3-add-slot .storage-v3-plus{position:absolute;width:20px;height:20px;display:grid;place-items:center;margin:23px 0 0 34px;border:2px solid #100f11;border-radius:50%;background:#2a262b;color:#b9b0ac;font-size:14px;font-weight:600;line-height:1}
    .storage-v3-add-icon{position:relative;display:grid;place-items:center}
    #storagePane.storage-v3-minimal .folder-add,#storagePane.storage-v3-minimal .inline-add{width:100%!important;margin-top:10px!important;border-radius:18px!important;background:#131114!important}
    #storagePane.storage-v3-minimal .folder-mode-note,#storagePane.storage-v3-minimal .folder-mode-option span{display:none!important}
    @media(max-width:980px){.storage-v3-add-slot{width:100%}}
    @media(max-width:650px){#storagePane.storage-v3-minimal .storage-v2-safety{min-height:164px!important}.storage-v3-add-slot{min-height:112px;border-radius:17px}.storage-v3-add-slot svg{width:38px;height:38px}}
  `;
  ...
  function tidyHero() {
    ...
    if (stateText === 'Protected') setText(state, 'All good');
    else if (stateText === 'Add a backup') setText(state, 'Backup needed');

    const metrics = [...pane.querySelectorAll('.storage-v2-metric')];
    const cloud = metrics.find(item => ['Cloud space','Server storage'].includes(item.querySelector(':scope > span')?.textContent));
    if (cloud) {
      const label = cloud.querySelector(':scope > span');
      setText(label, 'Server storage');
      const disk = serverStorageText?.textContent?.trim();
      if (disk) {
        const [used, total] = disk.split('/').map(part => part.trim());
        if (used) setText(cloud.querySelector('.storage-v2-ring strong'), used);
        if (total) setText(cloud.querySelector('.storage-v2-ring small'), `/ ${total}`);
      }
    }

    const integrity = metrics.find(item => ['Integrity','Mochimono files'].includes(item.querySelector(':scope > span')?.textContent));
    if (integrity) {
      integrity.classList.add('storage-v3-storage-check');
      const ring = integrity.querySelector('.storage-v2-ring');
      integrity.classList.toggle('warn', ring?.classList.contains('warn'));
      integrity.classList.toggle('bad', ring?.classList.contains('bad'));
      setText(integrity.querySelector(':scope > span'), 'Mochimono files');
      const value = ring?.querySelector('strong');
      if (value?.textContent.trim() === '100%') setText(value, '✓');
    }
  }

  function tidyFolders() {
    for (const row of folders?.querySelectorAll('[data-folder-path]') || []) {
      for (const node of row.querySelectorAll('.storage-v2-folder-node')) {
        const small = node.querySelector('small');
        if (!small) continue;
        let text = small.textContent.trim().replace(/^(Indexed|Synced)\s+/i, '');
  ...
  function tidy() { tidyHero(); tidyFolders(); }
  let frame = 0;
  const scheduleTidy = () => {
    if (frame) return;
    frame = requestAnimationFrame(() => { frame = 0; tidy(); });
  };
