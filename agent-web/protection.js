...
style.textContent = `
  ...
  .protection-controls{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
  .protection-controls select{width:auto;min-width:110px}
  ...
  .protection-levels{display:flex;gap:8px;flex-wrap:wrap;color:#8f8784;font-size:11px}
  ...
  .peer-row small{display:block;color:#77706e}.peer-online{color:#a9c9ae}.peer-offline{color:#b99186}
...
function locationLabel(location) {
  const bits = [];
  if (location.remote) bits.push('remote');
...
function openPeerDialog() {
  ...
  box.querySelector('#savePeer').onclick = async () => {
    ...
    const url = box.querySelector('#peerUrl').value.trim();
    const token = box.querySelector('#peerToken').value.trim();
    const site = box.querySelector('#peerSite').value.trim() || name;
    const reliability = box.querySelector('#peerReliability').value;
    try {
      await control('/api/client/protection/peers', { method:'POST', body: JSON.stringify({ name,url,token,site,reliability }) });
      closeDialog();
      await refresh(true);
      toast('Remote PC added');
    } catch (error) { toast(error.message); }
  };
  box.showModal();
}
...
function openShareDialog() {
  ...
  box.innerHTML = `<div class="dialog-head"><h3>Share backup space</h3><button class="icon" data-close>×</button></div>
    <div class="field-stack"><label class="field-label">Folder</label><div class="folder-path-row"><input id="sharePath" value="${esc(share.path || '')}" placeholder="Folder"><button id="shareChoose" class="secondary">Choose</button></div><label class="field-label">Space limit (GB, 0 = no limit)</label><input id="shareLimit" type="number" min="0" step="1" value="${share.maxBytes ? Math.round(share.maxBytes / 1e9) : 0}"><label><input id="shareEnabled" type="checkbox" ${share.enabled ? 'checked' : ''}> Allow this PC to store encrypted Mochimono backups</label>${live.enabled ? `<div class="folder-mode-note">${bytes(live.bytes)} stored · ${bytes(live.freeBytes)} free<br>${(live.urls || []).map(esc).join('<br>')}</div>` : ''}<button id="showPairing" class="secondary">Show pairing token</button><textarea id="pairingInfo" hidden readonly></textarea></div>
  ...
  box.querySelector('#saveShare').onclick = async () => {
    const path = box.querySelector('#sharePath').value.trim();
    const enabled = box.querySelector('#shareEnabled').checked;
    const maxBytes = Math.max(0, Number(box.querySelector('#shareLimit').value) || 0) * 1e9;
    try {
      await control('/api/client/protection/share', { method:'POST', body: JSON.stringify({ path, enabled, maxBytes }) });
      closeDialog();
      await refresh(true);
    } catch (error) { toast(error.message); }
  };
...
async function openTrashDialog() {
  ...
  box.showModal();
  try {
    const result = await server('/api/protection/trash');
    ...
    box.querySelector('#trashRestore').disabled = !files.length;
    box.querySelector('#trashEmpty').disabled = !files.length;
