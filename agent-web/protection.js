...
style.textContent = `
  .protection-dashboard{display:grid;gap:12px}
  .protection-summary{display:flex;align-items:baseline;gap:9px;flex-wrap:wrap}
  ...
  .protection-note{font-size:10px;color:#827b78;line-height:1.5}
  ...
  .peer-row small{display:block;color:#77706e;line-height:1.45}.peer-online{color:#a9c9ae}.peer-offline{color:#b99186}
...
function section() {
  ...
  if (backupSection) storagePane.insertBefore(element, backupSection);
  else storagePane.append(element);
...
function decorateFolders() {
  ...
  for (const row of folders?.querySelectorAll('[data-folder-path]') || []) {
    ...
    select.onchange = async () => {
      ...
      try {
        await control('/api/client/protection/folder-level', { method:'POST', body:JSON.stringify({ importId:folder.importId, level:select.value }) });
        await refresh(true);
...
function availabilityFor(location) {
  ...
  if (location.kind === 'peer') {
    ...
    return {
      ...
      text:peer?.online ? 'Expected backup drive reachable now' : (peer?.error || 'Remote backup is unreachable now')
    };
  }
  const ownSource = location.deviceName && location.deviceName === state?.config?.deviceName;
  return ownSource ? { state:'online', text:'This PC is online now' } : { state:'unknown', text:'Current availability unknown' };
}
...
function renderLocations() {
  return (state?.locations || []).map(location => {
    ...
    return `<div class="protection-location" data-location-id="${esc(location.id)}">
      <div><strong>${esc(location.name)}</strong><small>${esc(locationLabel(location))}${location.deviceName ? ` · ${esc(location.deviceName)}` : ''}<br><span class="location-${availability.state}">${esc(details)}</span></small></div>
...
function renderPeers() {
  ...
  return peers.map(peer => {
    ...
    return `<div class="peer-row" data-peer-id="${esc(peer.id)}"><div><strong>${esc(peer.name)}</strong><small>${esc(peer.site || 'Remote')} · encrypted · ${peer.enabled === false ? 'paused · ' : ''}<span class="${status?.online ? 'peer-online' : 'peer-offline'}">${esc(current)}</span>${esc(verified)}</small></div><button class="action-link" data-peer-toggle>${peer.enabled === false ? 'Resume' : 'Pause'}</button></div>`;
  }).join('');
}
...
function render() {
  ...
  if (!summary) {
    body.innerHTML = '<div class="error">Protection status unavailable</div>';
    return;
  }
  ...
  const jobText = job ? `<div class="protection-running"><span><strong>${esc(job.label || 'Protection')}</strong> · ${esc(p.phase || 'Working')} ${p.copied != null ? ` · ${Number(p.copied).toLocaleString()} copied` : ''}${p.copiedBytes ? ` · ${bytes(p.copiedBytes)}` : ''}${p.current ? ` · ${esc(p.current)}` : ''}</span><button id="cancelProtection" class="action-link">Pause</button></div>` : '';
  const uncertain = (state.locations || []).some(location => ['backup','peer'].includes(location.kind) && availabilityFor(location).state !== 'online');
  body.innerHTML = `
    <div class="protection-summary"><strong class="${summary.needsProtection ? 'warn' : 'good'}">${percent}% protected</strong><span>${esc(status)}</span></div>
    <div class="protection-meter" title="${summary.protectedFiles.toLocaleString()} of ${summary.files.toLocaleString()} files"><i style="width:${percent}%"></i></div>
    <div class="protection-levels">${LEVELS.map(([level,label]) => `<span>${label} <b>${Number(summary.levels?.[level]?.files || 0).toLocaleString()}</b>${summary.levels?.[level]?.needsProtection ? ` · ${Number(summary.levels[level].needsProtection).toLocaleString()} need protection` : ''}</span>`).join('')}</div>
    ${uncertain ? '<div class="protection-note">Offline backups are last-known copies. Mochimono remembers their last verification, but cannot know their current condition until the actual storage is reconnected.</div>' : ''}
    ${jobText}
    <div class="protection-controls"><span>Automatic</span><select id="protectionBackground"><option value="low">Low impact</option><option value="normal">Normal</option><option value="paused">Paused</option></select><button id="runProtection" class="secondary" ${job ? 'disabled' : ''}>Protect now</button><button id="addPeer" class="secondary">Add remote PC</button><button id="shareSpace" class="secondary">Share space</button><button id="backupKey" class="secondary">Recovery key</button>${summary.trash ? `<button id="openTrash" class="secondary">Trash · ${summary.trash.toLocaleString()}</button>` : ''}</div>
    <div class="storage-locations">${renderLocations()}</div>
    <div id="peerList">${renderPeers()}</div>`;
  body.querySelector('#protectionBackground').value = state.config.background;
  body.querySelector('#cancelProtection')?.addEventListener('click', async () => {
    try { await server('/api/job/cancel', { method:'POST', body:'{}' }); toast('Pausing protection…'); }
    catch (error) { toast(error.message); }
  });
  body.querySelector('#protectionBackground').onchange = async event => {
    try { await control('/api/client/protection/settings', { method:'POST', body:JSON.stringify({ background:event.target.value }) }); await refresh(true); }
    catch (error) { toast(error.message); }
  };
  body.querySelector('#runProtection').onclick = async () => {
    try { await control('/api/client/protection/run', { method:'POST', body:'{}' }); toast('Protection started'); }
    catch (error) { toast(error.message); }
  };
  ...
  body.querySelectorAll('[data-peer-toggle]').forEach(button => button.onclick = async () => {
    ...
    try {
      await control('/api/client/protection/peers/toggle', { method:'POST', body:JSON.stringify({ id, enabled:peer?.enabled === false }) });
      await refresh(true);
    } catch (error) { toast(error.message); }
  });
  decorateFolders();
...
async function saveLocation(event) {
  ...
  try {
    await control('/api/client/protection/location', { method:'POST', body:JSON.stringify({
      id,
      name:current.name,
      kind:current.kind,
      ...
      reliability:row.querySelector('[data-location-reliability]').value,
      site:row.querySelector('[data-location-site]').value.trim()
    }) });
    await refresh(true);
  } catch (error) { toast(error.message); }
}
...
function openPeerDialog() {
  ...
  box.innerHTML = `<div class="dialog-head"><h3>Add encrypted remote PC</h3><button class="icon" data-close>×</button></div>
    <div class="field-stack"><div class="folder-mode-note">Pair while the actual backup drive is connected. Mochimono binds this target to that drive, not just the PC.</div><input id="peerName" placeholder="Name"><input id="peerUrl" placeholder="http://friend-pc:8644"><input id="peerToken" placeholder="Pairing token"><input id="peerSite" placeholder="Physical location (optional)"><label class="field-label">Reliability</label><select id="peerReliability"><option value="normal">Normal</option><option value="high">Reliable</option><option value="low">Less reliable</option></select></div>
    <div class="dialog-actions"><div class="spacer"></div><button data-close class="secondary">Cancel</button><button id="savePeer" class="primary">Add</button></div>`;
  ...
  box.querySelector('#savePeer').onclick = async () => {
    ...
    try {
      await control('/api/client/protection/peers', { method:'POST', body:JSON.stringify({
        name,
        ...
        reliability:box.querySelector('#peerReliability').value
      }) });
      closeDialog();
      await refresh(true);
      toast('Remote backup drive paired');
    } catch (error) { toast(error.message); }
  };
  box.showModal();
}
  ...
  try {
    ...
    box.querySelector('#useRecoveryKey').onclick = async () => {
      ...
      try {
        ...
        toast('Recovery key saved'); closeDialog(); await refresh(true);
      } catch (error) {
        ...
        try {
          ...
          toast('Recovery key replaced'); closeDialog(); await refresh(true);
...
function openShareDialog() {
  ...
  const liveNote = !live.enabled ? '' : live.online
    ? `${bytes(live.bytes)} stored · ${bytes(live.freeBytes)} free · storage ${esc(String(live.storageId || '').slice(0, 8))}`
    : `Storage offline: ${esc(live.error || 'configured drive is unavailable')}`;
  box.innerHTML = `<div class="dialog-head"><h3>Share backup space</h3><button class="icon" data-close>×</button></div>
    <div class="field-stack"><div class="folder-mode-note">For an external drive, Mochimono writes a storage identity to the drive. If that drive is unplugged or replaced, the share becomes offline instead of falling back to another disk.</div><label class="field-label">Folder</label><div class="folder-path-row"><input id="sharePath" value="${esc(share.path || '')}" placeholder="Folder"><button id="shareChoose" class="secondary">Choose</button></div><label class="field-label">Space limit (GB, 0 = no limit)</label><input id="shareLimit" type="number" min="0" step="1" value="${share.maxBytes ? Math.round(share.maxBytes / 1e9) : 0}"><label><input id="shareEnabled" type="checkbox" ${share.enabled ? 'checked' : ''}> Allow this storage to hold encrypted Mochimono backups</label>${liveNote ? `<div class="folder-mode-note">${liveNote}${live.online ? `<br>${(live.urls || []).map(esc).join('<br>')}` : ''}</div>` : ''}<button id="showPairing" class="secondary">Show pairing token</button><textarea id="pairingInfo" hidden readonly></textarea></div>
    <div class="dialog-actions"><div class="spacer"></div><button data-close class="secondary">Cancel</button><button id="saveShare" class="primary">Save</button></div>`;
  box.querySelectorAll('[data-close]').forEach(button => button.onclick = closeDialog);
  box.querySelector('#shareChoose').onclick = async () => {
    try {
      const result = await fetch('/api/pick-folder').then(response => response.json());
  ...
  box.querySelector('#saveShare').onclick = async () => {
    try {
      await control('/api/client/protection/share', { method:'POST', body:JSON.stringify({
      ...
      closeDialog(); await refresh(true);
    } catch (error) { toast(error.message); }
  };
  box.showModal();
}
...
async function openTrashDialog() {
  ...
  try {
    ...
    box.querySelector('#trashRestore').onclick = async () => {
      try { await server('/api/protection/restore', { method:'POST', body:JSON.stringify({ hashes:files.map(file => file.hash) }) }); closeDialog(); await refresh(true); location.reload(); }
      catch (error) { toast(error.message); }
    };
    box.querySelector('#trashEmpty').onclick = async () => {
      ...
      try { await server('/api/protection/purge', { method:'POST', body:JSON.stringify({ hashes:files.map(file => file.hash), confirm:'DELETE' }) }); closeDialog(); await refresh(true); }
      catch (error) { toast(error.message); }
    };
...
async function refresh(force = false) {
  ...
  try {
    ...
    section();
    document.querySelector('#protectionBody').innerHTML = `<div class="error">${esc(error.message)}</div>`;
  }
}
...
document.addEventListener('click', event => {
  ...
  (async () => {
    try {
      ...
      if (cleanup.exclusiveFiles) trashExclusive = confirm(`${cleanup.exclusiveFiles.toLocaleString()} files (${bytes(cleanup.exclusiveBytes)}) are stored by Mochimono only because of this folder.\n\nMove those extra Mochimono copies to Trash too?\n\nOK = clean them up\nCancel = keep them in Mochimono`);
      await server(`/api/protection/imports/${folder.importId}/cleanup`, { method:'POST', body:JSON.stringify({ trashExclusive }) });
...
document.addEventListener('click', event => {
  ...
  (async () => {
    try {
      ...
      const estimate = await control('/api/client/protection/estimate-folder', { method:'POST', body:JSON.stringify({ path }) });
      const large = estimate.truncated || estimate.bytes >= 100e9 || estimate.files >= 100000;
      if (!large || confirm(`Protect ${estimate.truncated ? 'at least ' : ''}${estimate.files.toLocaleString()} files (${bytes(estimate.bytes)})?\n\nThis will create a Mochimono copy. You can stop protecting it later without deleting the original folder.`)) {
        button.dataset.protectionApproved = '1';
        button.click();
      }
    } catch (error) { toast(error.message); }
    finally { button.disabled = false; button.textContent = 'Protect'; }
  })();
}, true);
...
section();
refresh(true);
refreshTimer = setInterval(() => refresh(false), 12_000);
