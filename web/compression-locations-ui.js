...
`;
  ...
  async function writeMode(control) {
    ...
    if (mode === 'compact-only') {
      ...
      await api('/api/compression/storage-policy', { method:'POST', body:JSON.stringify({ locationId, mediaType, representation:mode }) });
      if (mode === 'compact') {
        await api('/api/compression/retention', { method:'POST', body:JSON.stringify({ locationId, mediaType, allowOriginalRemoval:false }) });
      }
    }
    window.dispatchEvent(new CustomEvent('mochimono:compression-policy-changed', { detail:{ locationId, mediaType, mode } }));
    return true;
  }

  async function load() {
    const [local, backups, snapshot] = await Promise.all([
      api('/api/client/locations').catch(() => ({ locations:[] })),
      api('/api/backups').catch(() => ({ backups:[] })),
      api('/api/compression/storage-snapshot').catch(() => ({ policies:[], retention:[] }))
    ]);
    const mode = modeMap(snapshot);
    ...
    dialog.querySelectorAll('select[data-location]').forEach(control => control.addEventListener('change', async () => {
      ...
      try {
        if (!await writeMode(control)) await load();
      } catch (error) {
        alert(error.message);
        await load();
