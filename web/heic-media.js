...
const record=(hash,filename='')=>({hash:String(hash||''),filename:String(filename||''),mime:'image/heic',kind:'image',urgent:true});

function cardRecord(card){
  const hash=String(card?.dataset?.hash||''),filename=String(card?.dataset?.filename||card?.title||'');
  return validHash(hash)&&HEIC_RE.test(filename)?record(hash,filename):null;
}
...
async function repair(item){
  ...
  if(!pending){
    pending=ensureBrowserThumbnail(item).then(async result=>{
      ...
      await Promise.all([invalidateVisual(item.hash),invalidateAi(item.hash)]);
