...
function ensureTinyPlaceholder(card) {
  if (!card || card.querySelector('.media-thumb') || !isVideoName(cardName(card))) return;
...
function replaceVideoElement(video) {
  ...
  enqueue(card.dataset.hash, true);
}
...
function enqueue(hash, front = false) {
  ...
  const blockedUntil = failedUntil.get(hash) || 0;
  if (blockedUntil > Date.now()) return;
  if (blockedUntil) failedUntil.delete(hash);
  queued.add(hash);
...
function waitFor(target, event, timeout = 6000) {
...
function decodedFrame(video, timeout = 1500) {
  if (!('requestVideoFrameCallback' in video)) return new Promise(resolve => setTimeout(resolve, 40));
...
function candidateTimes(duration) {
  ...
  return [...new Set([.2, 1, 4]
    .map(time => Math.min(end, time))
...
function retry(hash, error) {
  ...
  if (count >= MAX_ATTEMPTS || !cardFor(hash)) {
    attempts.delete(hash);
    failedUntil.set(hash, Date.now() + RETRY_COOLDOWN);
...
const rescanTimer = setInterval(scheduleScan, 1500);
scheduleScan();
