...
async function loadVisual(media){
  const byHash=new Map(media.map((item,index)=>[String(item.hash),index])),colors=Array(media.length).fill(null),robust=Array(media.length).fill(''),templates=Array(media.length).fill(null);
  let db;
  try{
    db=await openDb();
    if(!db.objectStoreNames.contains(VIS_STORE))return{colors,robust,templates};
    post('progress',{done:0,total:media.length,detail:'Reading perceptual color and duplicate fingerprints…',stage:'visual'});
    ...
    for(let scanned=0;scanned<rows.length;scanned++){
      ...
      if(row.robustVersion===ROBUST_VERSION&&ROBUST_RE.test(String(row.robust||'')))robust[index]=String(row.robust);
    ...
    return{colors,robust,templates};
  }catch(error){if(error?.name==='AbortError')throw error;return{colors,robust,templates}}
  finally{db?.close?.()}
}
function hammingHex(left,right){let total=0;for(let i=0;i<16;i++)total+=POPCOUNT16[parseInt(left.slice(i*4,i*4+4),16)^parseInt(right.slice(i*4,i*4+4),16)];return total}
...
function colorClose(a,b){
  ...
  return Math.abs(a.l-b.l)<=.065&&(hue<=.055||a.c<.025||b.c<.025);
}
function nearDuplicate(a,b,robust,templates,colors){
  const distance=hammingHex(robust[a],robust[b]);
...
function duplicateGroups(robust,templates,colors){
  const n=robust.length,parent=new Int32Array(n),size=new Int32Array(n),stamp=new Int32Array(n),heads=new Int32Array(DUP_BANDS*(1<<16)),next=new Int32Array(DUP_BANDS*n);
  heads.fill(-1);next.fill(-1);for(let i=0;i<n;i++){parent[i]=i;size[i]=1}
  const find=i=>{let root=i;while(parent[root]!==root)root=parent[root];while(parent[i]!==i){const nextParent=parent[i];parent[i]=root;i=nextParent}return root};
  ...
  for(let index=0;index<n;index++){
    abort();const hash=robust[index];if(!ROBUST_RE.test(hash))continue;
    let matchedExact=false,reps=exactReps.get(hash);
    if(reps){for(const candidate of reps){if(nearDuplicate(index,candidate,robust,templates,colors)){union(index,candidate);matchedExact=true;break}}}
    ...
    for(let band=0;band<DUP_BANDS;band++){
      ...
      for(let candidate=heads[headOffset];candidate>=0;candidate=next[band*n+candidate]){
        if(stamp[candidate]===generation)continue;stamp[candidate]=generation;
        if(robust[candidate]===hash)continue;
        if(nearDuplicate(index,candidate,robust,templates,colors))union(index,candidate);
        if(++comparisons%250000===0)abort();
      }
    }
    for(let band=0;band<DUP_BANDS;band++){
      const key=parseInt(hash.slice(band*4,band*4+4),16),headOffset=band*(1<<16)+key,nextOffset=band*n+index;
      next[nextOffset]=heads[headOffset];heads[headOffset]=index;
    }
  }
  const grouped=new Map();for(let i=0;i<n;i++){const root=find(i);if(size[root]<2)continue;let members=grouped.get(root);if(!members)grouped.set(root,members=[]);members.push(i)}
  const ids=new Int32Array(n);ids.fill(-1);let id=0,locked=0;
  for(const members of grouped.values()){for(const index of members)ids[index]=id;locked+=members.length;id++}
  return{ids,groups:[...grouped.values()],count:id,locked};
}
...
function hueSeam(colors){
  ...
  if(total<=0)return 0;
  let best=0,bestScore=Infinity;for(let bin=0;bin<bins;bin++){const score=hist[(bin+bins-2)%bins]*.25+hist[(bin+bins-1)%bins]*.7+hist[bin]*1.25+hist[(bin+1)%bins]*.7+hist[(bin+2)%bins]*.25;if(score<bestScore){bestScore=score;best=bin}}
  return(best+.5)/bins;
}
...
function axisTargets(colors,indexes,seam){const hueValue=index=>((colors[index].h-seam+1)%1),hueRank=rankMap(indexes,hueValue),lightRank=rankMap(indexes,index=>colors[index].l),tx=new Float32Array(colors.length),ty=new Float32Array(colors.length);for(const index of indexes){const hue=hueValue(index),light=colors[index].l;tx[index]=HUE_ABSOLUTE_WEIGHT*hue+(1-HUE_ABSOLUTE_WEIGHT)*hueRank[index];ty[index]=LIGHT_ABSOLUTE_WEIGHT*(1-light)+(1-LIGHT_ABSOLUTE_WEIGHT)*(1-lightRank[index])}return{tx,ty}}
function nearestFree(used,cols,rows,targetX,targetY,blocked=null){
  const available=(x,y)=>{const slot=y*cols+x;return!used[slot]&&!(blocked?.[slot])},cx=Math.max(0,Math.min(cols-1,Math.round(targetX))),cy=Math.max(0,Math.min(rows-1,Math.round(targetY)));
  if(available(cx,cy))return[cx,cy];const maxRadius=Math.max(cols,rows);let bestX=cx,bestY=cy,bestScore=Infinity;
  for(let radius=1;radius<=maxRadius;radius++){let found=false;const left=Math.max(0,cx-radius),right=Math.min(cols-1,cx+radius),top=Math.max(0,cy-radius),bottom=Math.min(rows-1,cy+radius),consider=(x,y)=>{if(!available(x,y))return;const dx=x-targetX,dy=y-targetY,score=dx*dx+dy*dy*1.2;if(score<bestScore){bestScore=score;bestX=x;bestY=y;found=true}};for(let x=left;x<=right;x++){consider(x,top);if(bottom!==top)consider(x,bottom)}for(let y=top+1;y<bottom;y++){consider(left,y);if(right!==left)consider(right,y)}if(found)return[bestX,bestY]}
  return blocked?nearestFree(used,cols,rows,targetX,targetY,null):[cx,cy];
}
function reserveCompactFamily(reserved,used,cols,rows,bounds,count){if(count<3||bounds.maxX<bounds.minX||bounds.maxY<bounds.minY)return false;const width=bounds.maxX-bounds.minX+1,height=bounds.maxY-bounds.minY+1,area=width*height;if(area>Math.ceil(count*1.55))return false;for(let py=bounds.minY;py<=bounds.maxY;py++)for(let px=bounds.minX;px<=bounds.maxX;px++){const slot=py*cols+px;if(!used[slot])reserved[slot]=1}return true}
async function loadFamilies(media){
  if(typeof Worker!=='function')return null;
  return new Promise(resolve=>{let settled=false;const finish=value=>{if(settled)return;settled=true;try{familyWorker.terminate()}catch{}resolve(value)},familyWorker=new Worker(new URL(`./ai-global-multimodal-worker.js?v=${FAMILY_WORKER_REV}`,import.meta.url),{type:'module'});familyWorker.onerror=()=>finish(null);familyWorker.onmessage=event=>{const data=event.data||{};if(data.type==='progress'){post('progress',{...data,detail:`AI families · ${data.detail||'matching related media…'}`});return}if(data.type==='result')finish(data.result||null);else if(data.type==='error')finish(null)};familyWorker.postMessage({action:'sort',payload:{media}})})
}
function effectiveFamilyIds(media,families,duplicates){
  const out=new Int32Array(media.length);out.fill(-1);let next=0;
  const duplicateMap=new Map();for(const members of duplicates.groups){const id=next++;for(const index of members){out[index]=id;duplicateMap.set(index,id)}}
  const groups=new Map();for(let index=0;index<media.length;index++){if(out[index]>=0)continue;const old=Number(families?.familyIds?.[index]);if(!Number.isInteger(old)||old<0)continue;let id=groups.get(old);if(id==null){id=next++;groups.set(old,id)}out[index]=id}
  return out;
}
function familyInfo(indexes,familyIds,order,media){
  ...
  const multi=[];for(const members of groups.values()){members.sort((a,b)=>orderRank[a]-orderRank[b]||a-b);if(members.length>1)multi.push(members);else singles.push(members[0])}multi.sort((a,b)=>b.length-a.length||orderRank[a[0]]-orderRank[b[0]]);singles.sort((a,b)=>orderRank[a]-orderRank[b]||a-b);return{multi,singles,orderRank};
}
function familyStrength(members,tx,ty){let mx=0,my=0;for(const index of members){mx+=tx[index];my+=ty[index]}mx/=members.length;my/=members.length;let spreadX=0,spreadY=0;for(const index of members){spreadX+=Math.abs(tx[index]-mx);spreadY+=Math.abs(ty[index]-my)}spreadX/=members.length;spreadY/=members.length;const xStrength=spreadX<.055?.88:spreadX<.11?.74:spreadX<.18?.52:spreadX<.28?.30:.10,yStrength=spreadY<.08?.72:spreadY<.18?.52:spreadY<.28?.30:.16;return{mx,my,xStrength,yStrength}}
function placeColorField(media,colors,indexes,seam,familyIds,order,x,y){
  ...
  for(const members of multi){abort();const family=familyStrength(members,tx,ty),bounds={minX:cols,minY:rows,maxX:-1,maxY:-1};for(let position=0;position<members.length;position++){const index=members[position],ring=position?Math.sqrt(position)*.0035:0,angle=position*2.399963229728653,nx=family.mx+(tx[index]-family.mx)*(1-family.xStrength)+Math.cos(angle)*ring,ny=family.my+(ty[index]-family.my)*(1-family.yStrength)+Math.sin(angle)*ring*.7,[px,py]=put(index,clamp(nx),clamp(ny));bounds.minX=Math.min(bounds.minX,px);bounds.maxX=Math.max(bounds.maxX,px);bounds.minY=Math.min(bounds.minY,py);bounds.maxY=Math.max(bounds.maxY,py)}if(reserveCompactFamily(reserved,used,cols,rows,bounds,members.length))reservedFamilies++}
  singles.sort((a,b)=>{const ca=colors[a],cb=colors[b],va=ca.c*5+ca.f,vb=cb.c*5+cb.f;return vb-va||Math.abs(ca.l-.5)-Math.abs(cb.l-.5)||String(media[a].hash).localeCompare(String(media[b].hash))});for(const index of singles){abort();put(index,tx[index],ty[index])}
...
function fitAspect(item,boxW,boxH){const ratio=mediaRatio(item),boxRatio=boxW/Math.max(.0001,boxH);return ratio>=boxRatio?[boxW,boxW/ratio]:[boxH*ratio,boxH]}
function duplicateBlockInfo(indexes,duplicateIds,logicalX,logicalY){
  const allowed=new Set(indexes),groups=new Map();
  for(const index of indexes){const id=Number(duplicateIds?.[index]);if(id<0)continue;let members=groups.get(id);if(!members)groups.set(id,members=[]);members.push(index)}
  const rowFor=new Map(),xFor=new Map();
  for(const[id,members]of groups){if(members.length<2)continue;let x=0,y=0;for(const index of members){x+=Number(logicalX[index])||0;y+=Number(logicalY[index])||0}x/=members.length;y/=members.length;rowFor.set(id,Math.round(y));xFor.set(id,x)}
  return{allowed,groups,rowFor,xFor};
}
function sortRow(row,duplicateIds,logicalX,blockInfo){
  const blocks=new Map(),singles=[];
  for(const index of row){const id=Number(duplicateIds?.[index]);if(id>=0&&blockInfo.groups.get(id)?.length>1){let members=blocks.get(id);if(!members)blocks.set(id,members=[]);members.push(index)}else singles.push(index)}
  const entries=singles.map(index=>({x:Number(logicalX[index])||0,members:[index]}));
  for(const[id,members]of blocks){members.sort((a,b)=>(Number(logicalX[a])||0)-(Number(logicalX[b])||0)||a-b);entries.push({x:blockInfo.xFor.get(id)??(Number(logicalX[members[0]])||0),members})}
  entries.sort((a,b)=>a.x-b.x||a.members[0]-b.members[0]);return entries.flatMap(entry=>entry.members);
}
function logicalRows(indexes,logicalX,logicalY,duplicateIds){
  const info=duplicateBlockInfo(indexes,duplicateIds,logicalX,logicalY),grouped=new Map();
  for(const index of indexes){const id=Number(duplicateIds?.[index]),key=id>=0&&info.rowFor.has(id)?info.rowFor.get(id):Math.round(Number(logicalY[index])||0);let row=grouped.get(key);if(!row)grouped.set(key,row=[]);row.push(index)}
  return[...grouped.entries()].sort((a,b)=>a[0]-b[0]).map(([,row])=>sortRow(row,duplicateIds,logicalX,info));
}
function packSection(media,indexes,logicalX,logicalY,duplicateIds,aspect,originX,outX,outY,renderW,renderH){
  if(!indexes.length)return{width:0,height:0,rows:0};
  const blockInfo=duplicateBlockInfo(indexes,duplicateIds,logicalX,logicalY),totalRatio=indexes.reduce((sum,index)=>sum+layoutRatio(media[index]),0),targetWidth=Math.max(4,Math.sqrt(Math.max(1,totalRatio)*Math.max(.7,Number(aspect)||1)));
  let rows=logicalRows(indexes,logicalX,logicalY,duplicateIds);
  if(rows.length>1){const merged=[];let pending=[],pendingRatio=0;const flush=()=>{if(!pending.length)return;merged.push(sortRow(pending,duplicateIds,logicalX,blockInfo));pending=[];pendingRatio=0};for(let rowIndex=0;rowIndex<rows.length;rowIndex++){const row=rows[rowIndex],rowRatio=row.reduce((sum,index)=>sum+layoutRatio(media[index]),0);if(rowRatio>=targetWidth*.46){flush();merged.push(row);continue}pending.push(...row);pendingRatio+=rowRatio;if(pendingRatio>=targetWidth*.70||rowIndex===rows.length-1)flush()}rows=merged}
...
async function build(media){
  ...
  const[visual,families]=await Promise.all([loadVisual(usable),loadFamilies(usable).catch(()=>null)]),{colors,robust,templates}=visual;abort();
  post('progress',{done:0,total:usable.length,detail:'Locking near-duplicate images together…',stage:'duplicates'});
  const duplicates=duplicateGroups(robust,templates,colors),familyIds=effectiveFamilyIds(usable,families,duplicates),neutralFlags=new Uint8Array(usable.length);
  for(let index=0;index<usable.length;index++)neutralFlags[index]=neutral(colors[index])?1:0;
  for(const members of duplicates.groups){let neutralVotes=0;for(const index of members)neutralVotes+=neutralFlags[index];const value=neutralVotes>members.length/2?1:0;for(const index of members)neutralFlags[index]=value}
  const colorful=[],neutrals=[];for(let index=0;index<usable.length;index++)(neutralFlags[index]?neutrals:colorful).push(index);
  ...
  const field=placeColorField(usable,colors,colorful,seam,familyIds,families?.order,logicalX,logicalY),neutralStart=field.cols?field.cols+NEUTRAL_GAP:0,neutralField=placeNeutrals(usable,colors,neutrals,neutralStart,familyIds,families?.order,logicalX,logicalY),reservedFamilies=field.reservedFamilies+neutralField.reservedFamilies;
  post('progress',{done:usable.length,total:usable.length,detail:'Justifying images into dense rows with duplicate blocks locked…',stage:'packing'});
  ...
  const colorPack=packSection(usable,colorful,logicalX,logicalY,duplicates.ids,COLOR_ASPECT,.04,x,y,renderW,renderH),neutralOrigin=.04+colorPack.width+(neutrals.length?PACK_SECTION_GAP:0),neutralPack=packSection(usable,neutrals,logicalX,logicalY,duplicates.ids,1,neutralOrigin,x,y,renderW,renderH),worldW=Math.max(1,.08+colorPack.width+(neutrals.length?PACK_SECTION_GAP+neutralPack.width:0)),worldH=Math.max(1,.08,Math.max(colorPack.height,neutralPack.height)+.08);
  return{kind:'map',x,y,renderW,renderH,worldW,worldH,labels:[],preserveRows:true,detail:`Color Map · adaptive hue × lightness · ${duplicates.count.toLocaleString()} near-duplicate groups locked · ${duplicates.locked.toLocaleString()} media · ${field.familyGroups.toLocaleString()} local families · ${reservedFamilies.toLocaleString()} compact family zones · ${neutrals.length.toLocaleString()} neutral / low-color`};
}
self.onmessage=async event=>{const data=event.data||{};if(data.action==='cancel'){canceled=true;return}if(data.action!=='build')return;canceled=false;try{const media=Array.isArray(data.payload?.media)?data.payload.media:[],result=await build(media);if(!canceled)post('result',{result})}catch(error){post('error',{error:error?.message||String(error),aborted:error?.name==='AbortError'})}};
