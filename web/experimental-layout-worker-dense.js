...
function makeRows(result,media,indexes,targetWidth){
  ...
  finish();
  if(rows.length>1){
    const last=rows.at(-1),lastSum=last.reduce((s,i)=>s+ratio(media[i]),0);
    if(lastSum<targetWidth*.36){
      ...
      let total=0;for(const i of merged)total+=ratio(media[i]);
      let cut=1,running=0,best=Infinity;
      for(let i=0;i<merged.length-1;i++){running+=ratio(media[merged[i]]);const error=Math.abs(running-total/2);if(error<best){best=error;cut=i+1}}
...
function packRows(result,media,indexes){
  const totalRatio=indexes.reduce((sum,index)=>sum+ratio(media[index]),0);
  ...
  for(let rowIndex=0;rowIndex<rows.length;rowIndex++){
    const row=rows[rowIndex],sum=row.reduce((s,index)=>s+ratio(media[index]),0),usable=Math.max(.1,innerWidth-GAP*Math.max(0,row.length-1));
    let height=usable/Math.max(.001,sum);
    if(rowIndex===rows.length-1&&height>1.35)height=1.35;
    let left=.5;
    for(const index of row){
      const width=ratio(media[index])*height;
      const centerX=left+width/2,centerY=top+height/2;
      x[index]=centerX-.5;y[index]=centerY-.5;renderW[index]=width;renderH[index]=height;
      left+=width+GAP;
    }
    top+=height+GAP;
  }
...
function run(payload){
  return new Promise((resolve,reject)=>{
    const worker=new Worker(new URL('./experimental-layout-worker.js?v=20260912-5',self.location.href),{type:'module'});child=worker;
    let done=false;
