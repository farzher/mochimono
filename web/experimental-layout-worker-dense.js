...
function loadMultimodalFamilies(media){return new Promise(resolve=>{const worker=new Worker(new URL('./ai-global-multimodal-worker.js?v=20260912-1',self.location.href),{type:'module'});child=worker;let done=false;const finish=value=>{if(done)return;done=true;if(child===worker)child=null;worker.terminate();resolve(value)};worker.onerror=()=>finish(null);worker.onmessageerror=()=>finish(null);worker.onmessage=event=>{const data=event.data||{};if(data.type==='progress'){post('progress',{...data,detail:data.detail?`Families · ${data.detail}`:'Finding multimodal families…'});return}if(data.type==='error')return finish(null);if(data.type!=='result')return;const source=data.result?.familyIds;if(!source?.length||source.length!==media.length)return finish(null);const ids=Int32Array.from(source),max=ids.reduce((value,id)=>Math.max(value,id),-1),sizes=new Uint32Array(Math.max(0,max+1));for(const id of ids)if(id>=0)sizes[id]++;let count=0;for(const size of sizes)if(size>1)count++;finish({ids,sizes,count})};worker.postMessage({action:'sort',payload:{media}})})}
function familyAffinity(families,colors,a,b){if(!families||a<0||b<0)return 0;const id=families.ids[a];if(id<0||id!==families.ids[b]||(families.sizes[id]||0)<2)return 0;const na=neutralAt(colors,a),nb=neutralAt(colors,b);if(na!==nb)return 0;const light=Math.abs(colors.l[a]-colors.l[b]);if(light>.18)return 0;const lightScore=1-light/.18;if(na&&nb)return .7+.3*lightScore;const hue=circularHueDistance(colors.h[a],colors.h[b]);if(hue>.14)return 0;return clamp(lightScore*.42+(1-hue/.14)*.58)}
function orderColorRow(items,rowIndex,cols,targetX,targetY,colors,dino,families,above){const base=items.slice().sort((a,b)=>targetX[a]-targetX[b]||targetY[a]-targetY[b]||a-b),pool=[],placed=[];let cursor=0;const window=Math.min(32,Math.max(12,Math.ceil(Math.sqrt(base.length)*1.6)));while(placed.length<base.length){abort();while(cursor<base.length&&pool.length<window)pool.push(base[cursor++]);const col=placed.length,position=base.length<=1?.5:col/(base.length-1),left=placed.length?placed.at(-1):-1,left2=placed.length>1?placed.at(-2):-1,up=above?.[col]??-1,upLeft=above?.[col-1]??-1,upRight=above?.[col+1]??-1;let best=0,bestScore=Infinity;for(let p=0;p<pool.length;p++){const candidate=pool[p];let score=Math.abs(targetX[candidate]-position)*4.1;if(left>=0)score+=localDist(colors,dino,left,candidate)*.62;if(up>=0)score+=localDist(colors,dino,up,candidate)*.40;score-=familyAffinity(families,colors,left,candidate)*1.35;score-=familyAffinity(families,colors,up,candidate)*.95;score-=familyAffinity(families,colors,left2,candidate)*.38;score-=familyAffinity(families,colors,upLeft,candidate)*.34;score-=familyAffinity(families,colors,upRight,candidate)*.30;if(neutralAt(colors,candidate)){const outside=Math.max(0,Math.abs(position-.5)-.16);score+=outside*4.5}if(score<bestScore){bestScore=score;best=p}}placed.push(pool.splice(best,1)[0])}return placed}
async function beautifulColorMap(media){const usable=Array.isArray(media)?media.filter(item=>HASH_RE.test(String(item?.hash||''))):[];if(!usable.length)return{kind:'map',x:new Float32Array(),y:new Float32Array(),worldW:1,worldH:1,labels:[],preserveRows:true,detail:'Color Map · no media'};const byHash=new Map(usable.map((item,index)=>[String(item.hash),index])),colors=await loadColors(usable,byHash);abort();const[dino,families]=await Promise.all([loadDino(usable,byHash),loadMultimodalFamilies(usable)]);abort();post('progress',{done:0,total:usable.length,detail:'Composing perceptual color field…',stage:'layout'});const seam=hueSeam(colors),targetX=new Float32Array(usable.length),targetY=new Float32Array(usable.length);for(let i=0;i<usable.length;i++){const light=colors.available[i]?colors.l[i]:.5;targetY[i]=1-clamp(light);if(neutralAt(colors,i))targetX[i]=.5+(hashNoise(String(usable[i].hash))-.5)*.14;else targetX[i]=((colors.h[i]-seam+1)%1)}const cols=Math.max(24,Math.ceil(Math.sqrt(usable.length*1.55))),rows=Math.max(1,Math.ceil(usable.length/cols)),ordered=Array.from({length:usable.length},(_,i)=>i).sort((a,b)=>targetY[a]-targetY[b]||targetX[a]-targetX[b]||a-b),x=new Float32Array(usable.length),y=new Float32Array(usable.length);x.fill(-1);y.fill(-1);let above=null;for(let row=0;row<rows;row++){abort();const chunk=ordered.slice(row*cols,Math.min(usable.length,(row+1)*cols));const placed=orderColorRow(chunk,row,cols,targetX,targetY,colors,dino,families,above);for(let col=0;col<placed.length;col++){x[placed[col]]=col;y[placed[col]]=row}above=placed;if(row&&row%40===0)post('progress',{done:Math.min(usable.length,(row+1)*cols),total:usable.length,detail:`Composing perceptual color field · ${Math.min(usable.length,(row+1)*cols).toLocaleString()} placed…`,stage:'layout'})}const neutralCount=Array.from({length:usable.length},(_,i)=>i).reduce((sum,i)=>sum+(neutralAt(colors,i)?1:0),0),familyText=families?.count?` · ${families.count.toLocaleString()} adaptive multimodal families guided`:'';return{kind:'map',x,y,worldW:cols,worldH:rows,labels:[],preserveRows:true,detail:`Color Map · OKLab lightness × adaptive hue · ${neutralCount.toLocaleString()} neutrals centered${familyText} · local visual AI refinement`}}
