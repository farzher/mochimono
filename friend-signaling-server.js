...
const PORT = Number(process.env.MOCHIMONO_SIGNAL_PORT || 8645);
const HOST = process.env.MOCHIMONO_SIGNAL_HOST || process.env.HOST || '127.0.0.1';
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (!await handle(req, res, url)) json(res, 404, { error: 'Not found' });
  } catch (error) {
...
server.listen(PORT, HOST, () => console.log(`Mochimono friend signaling: http://${HOST}:${PORT}${PREFIX}`));
