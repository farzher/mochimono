...
const MIME = new Map([
  ...
  ['.ts', 'text/typescript'], ['.tsx', 'text/typescript'], ['.cts', 'text/typescript'],
  ['.json', 'application/json'], ['.pdf', 'application/pdf'], ['.zip', 'application/zip'], ['.7z', 'application/x-7z-compressed'], ['.rar', 'application/vnd.rar']
]);

const typescriptDeclaration = path => /\.d\.(?:mts|cts|ts)$/i.test(String(path || ''));

export function mimeFor(path, supplied = '') {
  if (typescriptDeclaration(path)) return 'text/typescript';
  return String(supplied || '').trim() || MIME.get(extname(path).toLowerCase()) || 'application/octet-stream';
}

export function mediaScope(value) {
  return String(value || '').toLowerCase() === 'all' ? 'all' : 'media';
}

export function isMediaFile(path, supplied = '') {
  const mime = mimeFor(path, supplied);
