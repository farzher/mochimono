...
function pathYearDate(value) {
  const match = String(value || '').replaceAll('\\', '/').match(/(?:^|\/)((?:19|20)\d{2})(?:\/|$)/);
  return match ? validDate(Number(match[1]), 1, 1) : 0;
}
...
export function localDisplayDate(value, mtimeMs, birthtimeMs = 0) {
  ...
  const pathYear = pathYearDate(value);
  if (pathYear) return { ms: pathYear, source: 'folder.year' };

  const created = plausible(birthtimeMs);
  if (created) return { ms: created, source: 'filesystem.created' };

  return { ms: Date.now(), source: 'filesystem.unknown' };
}
