...
export function openSyncIndex(path) {
  ...
  const page = db.prepare('SELECT path, size, mtime_ms AS mtimeMs, hash FROM file_hashes WHERE root = ? ORDER BY path LIMIT ? OFFSET ?');
  ...
  return {
    ...
    page(root, offset = 0, limit = 256) {
      const safeOffset = Math.max(0, Number(offset) || 0);
      const safeLimit = Math.max(1, Math.min(1000, Number(limit) || 256));
      return page.all(root, safeLimit, safeOffset);
    },
    ...
    forget(root, path) {
      if (confirmedAbsent(root, path)) removeEntry.run(root, path);
    },
    prune(root, currentPaths) {
      ...
      // error. Only remove unseen rows when the filesystem explicitly confirms
      // that their paths no longer exist.
