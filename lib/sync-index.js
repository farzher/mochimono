...
export function openSyncIndex(path) {
  ...
  const catalogVersionPath = `${path}.catalog-version`;
  let catalogRevision = 0;
  if (!existsSync(catalogVersionPath)) {
    // This sidecar changes only when file_hashes changes. SQLite/WAL timestamps
    // also change for root_meta/preview_meta bookkeeping, which must not make the
    // browser believe the entire file catalog changed.
    writeFileSync(catalogVersionPath, `${Date.now()}:${process.pid}:0\n`);
  }
  const bumpCatalogVersion = () => {
    catalogRevision++;
    writeFileSync(catalogVersionPath, `${Date.now()}:${process.pid}:${catalogRevision}\n`);
  };
  ...
  return {
    ...
    save(root,path,size,mtimeMs,hash) {
      const result = save.run(root,path,Number(size),Math.trunc(Number(mtimeMs)),hash);
      if (result.changes) bumpCatalogVersion();
    },
    saveMany(root,rows) {
    ...
    forget(root,path) {
      ...
      const changed = Number(removeEntry.run(root,path).changes) || 0;
      if (changed) bumpCatalogVersion();
      return changed;
    },
    forgetPrefix(root,prefix) {
      ...
      const changed = Number(removePrefix.run(root,start,`${start}\uffff`).changes) || 0;
      if (changed) bumpCatalogVersion();
      return changed;
    },
    ...
    moveRoot(from,to) {
      ...
      if (rows.length) bumpCatalogVersion();
      return rows.length;
    },
    forgetRoot(root) {
      const changed = Number(removeRoot.run(root).changes) || 0;
      removeMeta.run(root);
      removePreviewMeta.run(root);
      if (changed) bumpCatalogVersion();
    },
