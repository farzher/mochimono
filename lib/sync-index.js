...
export function openSyncIndex(path) {
  ...
  function confirmedHashesSet(hashes, outsideBrowse = false) {
    ...
    for (let offset = 0; offset < wanted.length; offset += 400) {
      ...
      const scope = outsideBrowse ? "root NOT LIKE 'browse:%' AND " : '';
      for (const row of db.prepare(`SELECT DISTINCT content_hash AS hash FROM files WHERE ${scope}content_hash IN (${placeholders})`).all(...chunk)) found.add(String(row.hash));
    }
    return found;
  }
  ...
  return {
    ...
    saveMany(root,rows) {
      ...
      try {
        ...
        for (const row of rows) {
          const value = String(row.contentHash || row.hash || row.id || '');
    ...
    duplicateBrowseContentHashes(hashes = null) {
      ...
      for (const hash of confirmedHashesSet(candidates, true)) duplicates.add(hash);
      for (const hash of backupHashesSet(candidates)) duplicates.add(hash);
      return duplicates;
    },
    knownContentHashes() {
      const hashes = new Set(db.prepare("SELECT DISTINCT content_hash AS hash FROM files WHERE length(content_hash) = 64").all().map(row => String(row.hash).toLowerCase()));
      for (const root of settings.backups || []) {
        const catalog = join(root, '.mochimono', 'catalog.sqlite');
        if (!existsSync(catalog)) continue;
        let backup;
        try {
          backup = new DatabaseSync(catalog, { readOnly:true, timeout:1000 });
          for (const row of backup.prepare('SELECT hash FROM objects').iterate()) hashes.add(String(row.hash).toLowerCase());
        } catch {
        } finally {
          try { backup?.close(); } catch {}
        }
    ...
    promoteAllBrowseHashes(root) {
      const pending = Number(browseStateStats.get(root)?.pending) || 0;
