...
export function openSyncIndex(path) {
  ...
  return {
    ...
    knownContentHashes() {
      const hashes = new Set(db.prepare("SELECT DISTINCT content_hash AS hash FROM files WHERE length(content_hash) = 64").all().map(row => String(row.hash)));
      for (const root of settings.backups || []) {
        const inventory = join(root, '.mochimono', 'inventory.sqlite');
        if (!existsSync(inventory)) continue;
        let backup;
        try {
          backup = new DatabaseSync(inventory, { readOnly:true, timeout:1000 });
          for (const row of backup.prepare('SELECT hash FROM objects').iterate()) hashes.add(String(row.hash));
        } catch {
        } finally {
          try { backup?.close(); } catch {}
        }
