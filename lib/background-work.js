...
function status(probe) {
  ...
  // Status rendering is passive. Only an actual background-work decision probes
  // CPU/session idle state, so an open Storage screen costs no OS subprocesses.
  if (probe) refreshIdleSignals();
...
export const backgroundWorkStatus = () => status(false);
export const backgroundWorkAllowed = () => status(true).allowed;
