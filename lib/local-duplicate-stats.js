...
export function localDuplicateStats() {
  ...
  try {
    ...
    const rows = db.prepare(`
      SELECT content_hash AS hash, MAX(size) AS size, COUNT(*) AS copies
      FROM files
      WHERE root IN (${placeholders}) AND content_hash <> ''
    ...
    return {
      sourceBytes:Number(totals?.bytes) || 0,
      sourceFiles:Number(totals?.files) || 0,
      duplicateBytes,
      duplicateFiles,
