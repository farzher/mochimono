...
async function localBackupSpace(path) {
  ...
  return { freeBytes, reserveBytes, budgetBytes: Math.max(0, freeBytes - reserveBytes) };
}
...
async function protectLocalBackup(backup, update, mode) {
  ...
  const space = await localBackupSpace(backup.path);
