...
async function peerStatus(peer, { allowUnbound = false } = {}) {
  ...
  if (!allowUnbound && (!peer.storageId || peer.storageId !== status.storageId)) {
    throw new Error('A different backup drive is connected to this remote PC');
  }
  return status;
}
...
async function preparePeerStorage(value) {
  ...
  if (identity?.storageId) {
    if (value.share.storageId && value.share.storageId !== identity.storageId) {
      throw Object.assign(new Error('A different storage device is mounted at this backup path'), { status: 409 });
    }
...
async function peerStorageAvailability(value) {
  ...
  if (!value.share.storageId || identity.storageId !== value.share.storageId) {
    return { enabled: true, online: false, storageId: value.share.storageId, error: 'A different storage device is mounted at this path' };
  }
  if (!existsSync(join(peerDataRoot(value), 'inventory.sqlite'))) {
    return { enabled: true, online: false, storageId: value.share.storageId, error: 'Backup storage inventory is missing' };
  }
...
export async function handleProtectionAgent(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/peers') {
    ...
    let remote;
    try { remote = await peerStatus(probe, { allowUnbound: !existing }); }
    catch (error) return void json(res, 409, { error: error.message });
    if (existing?.storageId && existing.storageId !== remote.storageId) {
      return void json(res, 409, { error: 'This remote PC is presenting a different backup drive. Add it as new storage instead of reusing the old backup identity.' });
    }
