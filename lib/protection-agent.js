...
import { api, beginJob, canceled, CONFIG_DIR, currentJob, forgetBackupId, json, now, queueJob, readJson, settings, SYNC_INDEX_PATH } from './agent-context.js';
import { backupLocations } from './agent-backups.js';
import { localCandidate, localLocations, protectionIntents } from './local-locations.js';
import { syncIndexRevision } from './sync-index.js';
import { syncProtectedFolders } from './agent-sync.js';
...
const SOURCE_SYNC_MS = 30 * 60_000;
const LOCAL_BACKUP_RESERVE_MIN = 32 * 1024 * 1024;
const LOCAL_BACKUP_RESERVE_MAX = 256 * 1024 * 1024;
let config = null;
...
async function loadConfig() {
  ...
  config = {
    ...
    peers: Array.isArray(saved.peers) ? saved.peers.filter(peer => peer?.id && peer?.url && peer?.token).map(peer => ({
      ...
      storageId: String(peer.storageId || ''),
    ...
    share: {
      ...
      storageId: String(saved.share?.storageId || '')
    }
  };
  await persist();
  return config;
...
function publicConfig(value) {
  return {
    ...
    share: {
      ...
      storageId: value.share.storageId,
...
async function peerStatus(peer, { allowUnbound = false } = {}) {
  const response = await peerRequest(peer, '/v1/status');
  const status = await response.json();
  if (status.online === false) throw new Error(status.error || 'Remote backup storage is offline');
  if (!status.storageId) throw new Error('Remote backup storage has no identity');
  if (!allowUnbound && (!peer.storageId || peer.storageId !== status.storageId)) throw new Error('A different backup drive is connected to this remote PC');
  return status;
}
...
async function registerPeer(peer) {
  await api('/api/drives/register', {
    ...
    body: {
      ...
      name: peer.name,
      storage: {
        name: peer.name, kind: 'peer', deviceName: `${peer.name}:${peer.storageId || peer.id}`, site: peer.site,
...
async function syncLocalBackupLocations(value) {
  for (const id of settings.disconnectedBackups || []) {
    await api(`/api/drives/${encodeURIComponent(id)}`, { method:'DELETE' }).catch(() => {});
  }
...
async function syncSourceReplicas(value, force = false) {
  const revision=syncIndexRevision(SYNC_INDEX_PATH);
  if (!force && revision===lastSourceRevision && Date.now() - lastSourceSyncAt < SOURCE_SYNC_MS) return;
  ...
  sourceSyncPromise = (async () => {
    ...
    const intents=protectionIntents();
    const scanId = randomUUID();

    for (let offset = 0; offset < intents.length; offset += 1500) {
      canceled();
      await api(`/api/protection/intents/${encodeURIComponent(settings.device)}`, {
        method:'POST',
        body:{ scanId, entries:intents.slice(offset,offset+1500) }
      });
    }
    await api(`/api/protection/intents/${encodeURIComponent(settings.device)}`, {
      method:'POST',
      body:{ scanId, entries:[], final:true }
    });
...
async function uploadPeerObject(peer, object, update) {
  ...
  await peerRequest(peer, `/v1/objects/${opaqueId(object.hash)}`, {
    method: 'PUT',
...
async function verifyEncryptedPeerObject(peer, object) {
  ...
  try {
    ...
    const decipher = createDecipheriv('aes-256-gcm', objectKey(object.hash), prefix.subarray(HEADER.length));
    decipher.setAuthTag(tag);
    const digest = createHash('sha256');
    let bytes = 0;
    const source = createReadStream(encrypted, { start: HEADER_BYTES, end: HEADER_BYTES + Number(object.size) - 1 }).pipe(decipher);
    for await (const chunk of source) {
      canceled();
      digest.update(chunk);
      bytes += chunk.length;
    }
    if (bytes !== Number(object.size) || digest.digest('hex') !== object.hash) throw new Error(`Decrypted backup verification failed for ${object.hash}`);
    return true;
  } finally {
...
async function localBackupSpace(path, quotaBytes = 0, usedBytes = 0) {
  const fs = await statfs(path);
  const freeBytes = Math.max(0, Number(fs.bavail) * Number(fs.bsize));
  const reserveBytes = Math.min(
    freeBytes,
    Math.min(LOCAL_BACKUP_RESERVE_MAX, Math.max(LOCAL_BACKUP_RESERVE_MIN, Math.floor(freeBytes * .02)))
  );
  const diskBudget = Math.max(0, freeBytes - reserveBytes);
  const quota = Math.max(0, Number(quotaBytes) || 0);
  const quotaBudget = quota ? Math.max(0, quota - Math.max(0, Number(usedBytes) || 0)) : diskBudget;
  return { freeBytes, reserveBytes, budgetBytes:Math.min(diskBudget, quotaBudget), quotaBytes:quota };
}

function noSpace(error) {
  return error?.code === 'ENOSPC' || /no space|not enough free space|disk full|ENOSPC/i.test(String(error?.message || ''));
}
...
async function protectLocalBackup(backup, update, mode) {
  const id = backup.meta?.id;
  if (!id) return { copied: 0, copiedBytes: 0, already: 0, skipped: 0, skippedBytes: 0, capacityLimited: false };
  const name = backup.meta?.name || basename(backup.path);
  const space = await localBackupSpace(backup.path, backup.meta?.quotaBytes, backup.local?.bytes);
  let budgetBytes = space.budgetBytes;
  const db = openBackupInventory(backup.path);
      ...
      for (const object of page.objects || []) {
        ...
        if (local && existsSync(destination)) {
          try {
            ...
            present = info.isFile() && Number(info.size) === size && await hashFile(destination) === object.hash;
          } catch {}
        }

        if (!present && size > budgetBytes) {
          skipped++;
          skippedBytes += size;
          capacityLimited = true;
          update({
            phase: `Filling available ${name} space`, current: object.hash.slice(0, 12),
            copied, copiedBytes, already, skipped, skippedBytes,
            remainingCapacityBytes: budgetBytes, capacityLimited: true
          });
          continue;
        }
        ...
        if (!present) {
          update({
            phase: `Backing up to ${name}`, current: object.hash.slice(0, 12),
            copied, copiedBytes, already, skipped, skippedBytes,
            remainingCapacityBytes: budgetBytes
          });
          try {
            await downloadPrimary(object.hash, size, destination);
          } catch (error) {
            if (!noSpace(error)) throw error;
            skipped++;
            skippedBytes += size;
            capacityLimited = true;
            budgetBytes = 0;
            update({
              phase: `${name} is full · using other storage`, current: object.hash.slice(0, 12),
              copied, copiedBytes, already, skipped, skippedBytes,
              remainingCapacityBytes: 0, capacityLimited: true
            });
            continue;
          }
          copied++;
          copiedBytes += size;
          budgetBytes = Math.max(0, budgetBytes - size);
        } else already++;

        save.run(object.hash, size, timestamp, timestamp);
  ...
  if (touched) {
    try { await saveBackupCatalog(backup.path); }
    catch (error) {
      if (!noSpace(error)) throw error;
      capacityLimited = true;
      update({
        phase: `${name} is full · existing catalog kept`, copied, copiedBytes, already,
        skipped, skippedBytes, remainingCapacityBytes: budgetBytes, capacityLimited: true
      });
    }
  }
  return { copied, copiedBytes, already, skipped, skippedBytes, capacityLimited, remainingCapacityBytes: budgetBytes };
}
...
async function reportPeerVerification(peer, good, bad) {
  for (let offset = 0; offset < good.length; offset += 1000) {
    await api(`/api/drives/${encodeURIComponent(peer.id)}/replicas`, { method:'POST', body:{ replicas:good.slice(offset, offset + 1000) } });
  }
  for (let offset = 0; offset < bad.length; offset += 1000) {
    await api(`/api/drives/${encodeURIComponent(peer.id)}/replicas/remove`, { method:'POST', body:{ hashes:bad.slice(offset, offset + 1000) } });
  }
}

async function verifyPeer(peer, update) {
  await peerStatus(peer);
  let after = '';
  let checked = 0;
  let verified = 0;
  ...
  do {
    canceled();
    const page = await api(`/api/drives/${encodeURIComponent(peer.id)}/files?after=${encodeURIComponent(after)}&limit=200`);
    for (const object of page.files || []) {
      canceled();
      update({ phase:`Verifying ${peer.name}`, current:object.hash.slice(0, 12), checked, verified, bad });
      try {
        await verifyEncryptedPeerObject(peer, object);
        good.push({ hash:object.hash, verifiedAt:now() });
        verified++;
      } catch (error) {
        if (error.canceled) throw error;
        failed.push(object.hash);
        bad++;
      }
      checked++;
      if (good.length + failed.length >= 100) {
        await reportPeerVerification(peer, good.splice(0), failed.splice(0));
      }
      if (config?.background === 'low') await sleep(80);
    }
    after = page.nextAfter || '';
  } while (after);
  if (good.length || failed.length) await reportPeerVerification(peer, good, failed);
  update({ phase:'Done', checked, verified, bad });
  return { checked, verified, bad };
}
...
function targetMet(target, copies) {
  ...
  const originals = qualified.filter(copy => copy.representation !== 'compact').length;
  ...
  return originals >= 1 && qualified.length >= Number(target?.copies || 0) && devices.size >= Number(target?.devices || 0) && sites.size >= Number(target?.sites || 0) && remote >= Number(target?.remote || 0);
}

async function reachableManagedCopies(state, value) {
  const attached = new Set((await backupLocations()).map(item => item.meta?.id).filter(Boolean));
  const onlinePeers = new Set();
  for (const peer of value.peers.filter(item => item.enabled !== false)) {
    try { await peerStatus(peer); onlinePeers.add(peer.id); } catch {}
  }
  return (state.copies || []).filter(copy => {
...
async function runProtection(update, automatic = false) {
  ...
  try {
    ...
    for (const backup of backups) {
      ...
      if (result.capacityLimited) capacityLimitedTargets++;
    ...
    update({
      phase: skipped ? 'Protected what fits · more storage needed' : 'Done',
      copied, copiedBytes, skipped, skippedBytes, capacityLimited: Boolean(skipped)
    });
    return {
      copied, copiedBytes, skipped, skippedBytes, capacityLimitedTargets,
      deletedSources, primaryRepaired, targets: backups.length + enabledPeers.length
    };
  } finally {
...
async function freeLocal(hash, value) {
  ...
  const reachable = await reachableManagedCopies(state, value);
  if (!targetMet(state.target, reachable)) {
    const error = new Error('The protection target currently depends on an offline or unreachable copy. Reconnect it or create another reachable verified copy first.');
    error.status = 409;
  ...
  if (localCount <= 1) {
    await api(`/api/protection/source-replicas/${encodeURIComponent(settings.device)}/remove`, { method: 'POST', body: { hashes: [hash] } }).catch(() => {});
  }
...
const peerControlRoot = value => join(shareRoot(value), '.mochimono');
const peerDataRoot = value => join(peerControlRoot(value), 'peer-share');
const peerIdentityPath = value => join(peerControlRoot(value), 'peer-storage.json');
const peerObjectPath = (value, id) => join(peerDataRoot(value), 'objects', id.slice(0, 2), id);

function peerDb(value, create = false) {
  const path = join(peerDataRoot(value), 'inventory.sqlite');
  if (!create && !existsSync(path)) throw new Error('Peer storage inventory is missing');
...
async function readPeerIdentity(value) {
  try {
    const data = JSON.parse(await readFile(peerIdentityPath(value), 'utf8'));
    return data?.storageId ? data : null;
  } catch { return null; }
}

async function preparePeerStorage(value) {
  const root = shareRoot(value);
  const info = root ? await stat(root).catch(() => null) : null;
  if (!root || !info?.isDirectory()) throw Object.assign(new Error('The shared backup folder is not currently connected'), { status: 409 });
  const markerExists = existsSync(peerIdentityPath(value));
  let identity = await readPeerIdentity(value);
  if (identity?.storageId) {
    if (value.share.storageId && value.share.storageId !== identity.storageId) throw Object.assign(new Error('A different storage device is mounted at this backup path'), { status: 409 });
    value.share.storageId = String(identity.storageId);
  } else {
    if (markerExists) throw Object.assign(new Error('The backup storage identity file is damaged'), { status: 409 });
    if (value.share.storageId) throw Object.assign(new Error('The configured backup drive is missing. Choose a different folder to configure replacement storage.'), { status: 409 });
    await mkdir(peerControlRoot(value), { recursive: true });
    value.share.storageId = randomUUID();
    identity = { format: 1, storageId: value.share.storageId, createdAt: now() };
    await writeFile(peerIdentityPath(value), `${JSON.stringify(identity, null, 2)}\n`, { flag: 'wx' });
  }
  await mkdir(peerDataRoot(value), { recursive: true });
  peerDb(value, true).close();
  await persist();
  return identity;
}

async function peerStorageAvailability(value) {
  if (!value.share.enabled || !shareRoot(value)) return { enabled: false, online: false };
  const root = shareRoot(value);
  const info = await stat(root).catch(() => null);
  if (!info?.isDirectory()) return { enabled: true, online: false, storageId: value.share.storageId, error: 'Configured backup storage is not connected' };
  const identity = await readPeerIdentity(value);
  if (!identity?.storageId) return { enabled: true, online: false, storageId: value.share.storageId, error: 'Backup storage identity is missing or damaged' };
  if (!value.share.storageId || identity.storageId !== value.share.storageId) return { enabled: true, online: false, storageId: value.share.storageId, error: 'A different storage device is mounted at this path' };
  if (!existsSync(join(peerDataRoot(value), 'inventory.sqlite'))) return { enabled: true, online: false, storageId: value.share.storageId, error: 'Backup storage inventory is missing' };
  return { enabled: true, online: true, storageId: value.share.storageId };
}

async function shareStatus(value) {
  const availability = await peerStorageAvailability(value);
  if (!availability.enabled || !availability.online) return { ...availability, name: value.share.name, maxBytes: Number(value.share.maxBytes) || 0 };
  try {
    ...
    return { enabled: true, online: false, storageId: value.share.storageId, name: value.share.name, maxBytes: Number(value.share.maxBytes) || 0, error: error.message };
  }
}
...
async function handlePeerRequest(req, res) {
  ...
  if (req.method === 'GET' && url.pathname === '/v1/status') return void json(res, 200, status);
  if (!status.online) return void json(res, 503, { error: status.error || 'Peer backup storage is offline' });
  ...
  if (req.method === 'PUT') {
    ...
    const projected = Number(status.bytes || 0) - Number(previous?.size || 0) + length;
    if (status.maxBytes > 0 && projected > status.maxBytes) return void json(res, 507, { error: 'Peer storage limit reached' });
    if (length > Number(status.freeBytes || 0) + Number(previous?.size || 0)) return void json(res, 507, { error: 'Not enough free space' });
...
async function localState() {
  ...
  const peerStates = await Promise.all(value.peers.map(async peer => {
    try {
      const remote = await peerStatus(peer);
      return { id: peer.id, online: true, ...remote };
    } catch (error) { return { id: peer.id, online: false, storageId: peer.storageId, error: error.message }; }
  }));
  return {
    ...
    backups: backups.map(item => ({
      id:item.meta?.id, name:item.meta?.name, path:item.path,
      bytes:Number(item.local?.bytes) || 0, files:Number(item.local?.count) || 0,
      freeBytes:Number(item.freeBytes) || 0, quotaBytes:Number(item.meta?.quotaBytes) || 0
    })),
...
export async function handleProtectionAgent(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/run') {
    const job = queueJob('protection', 'Protect library', async update => {
      await syncProtectedFolders(update);
      return runProtection(update,false);
    });
    json(res, 202, { job });
    return true;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/backup/forget') {
    const body = await readJson(req);
    const id = String(body.id || '');
    if (!id) return void json(res, 400, { error:'Backup drive required' });
    const attached = (await backupLocations()).some(item => String(item.meta?.id || '') === id);
    if (attached) return void json(res, 409, { error:'Disconnect this drive from its Storage card while it is attached.' });
    await forgetBackupId(id);
    await api(`/api/drives/${encodeURIComponent(id)}`, { method:'DELETE' });
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/location') {
    ...
    if (peer) {
      if (body.site !== undefined) peer.site = String(body.site || peer.name).trim().slice(0, 120) || peer.name;
      if (['low','normal','high'].includes(body.reliability)) peer.reliability = body.reliability;
      await persist();
    }
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/peers') {
    ...
    const probe = {
      id: existing?.id || `peer-${randomUUID()}`,
    ...
    let remote;
    try { remote = await peerStatus(probe, { allowUnbound: !existing?.storageId }); }
    catch (error) { return void json(res, 409, { error: error.message }); }
    if (existing?.storageId && existing.storageId !== remote.storageId) return void json(res, 409, { error: 'This remote PC is presenting a different backup drive. Add it as new storage instead of reusing the old backup identity.' });
    probe.storageId = remote.storageId;
    if (existing) Object.assign(existing, probe);
    else value.peers.push(probe);
    await persist();
    try { await registerPeer(existing || probe); } catch {}
    const saved = existing || probe;
    json(res, 200, { ok: true, peer: { ...saved, token: undefined, hasToken: true } });
    return true;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/peers/verify') {
    const body = await readJson(req);
    const peer = value.peers.find(item => item.id === String(body.id || ''));
    if (!peer) return void json(res, 404, { error: 'Remote backup not found' });
    const job = queueJob('protection', `Verify ${peer.name}`, update => verifyPeer(peer, update));
    json(res, 202, { job });
    return true;
  }
  if (req.method === 'POST' && url.pathname === '/api/client/protection/share') {
    ...
    if (body.path !== undefined) {
      const nextPath = body.path ? resolve(String(body.path)) : '';
      if (nextPath !== value.share.path) value.share.storageId = '';
      value.share.path = nextPath;
    }
    ...
    if (value.share.enabled) {
      ...
      await preparePeerStorage(value);
    } else await persist();
  ...
  if (req.method === 'GET' && url.pathname === '/api/client/protection/share-token') {
    json(res, 200, { token: value.share.token, urls: networkUrls(), storageId: value.share.storageId });
    return true;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/client/protection/key') {
    ...
    if (key !== value.backupKey && value.peers.some(peer => peer.lastBackupAt) && body.confirm !== 'REPLACE') return void json(res, 409, { error: 'Replacing this key makes existing remote backups unreadable. Confirm REPLACE only when restoring the original saved key.', requiresConfirmation: true });
    value.backupKey = key;
    await persist();
...
export async function startProtectionAgent() {
  ...
  await updatePeerServer(value);
  const firstSourceSync=setTimeout(() => syncSourceReplicas(value,true).catch(error => console.warn('Protection source sync failed:',error.message)), 1_000);
  firstSourceSync.unref?.();
