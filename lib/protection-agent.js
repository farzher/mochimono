...
async function updatePeer(peer, update, mode) {
  const statusResponse = await peerRequest(peer, '/v1/status');
...
async function reachableManagedCopies(state, value) {
  ...
  for (const peer of value.peers.filter(item => item.enabled !== false)) {
    try {
      ...
      if (status.online !== false) onlinePeers.add(peer.id);
    } catch {}
  }
...
async function peerStorageAvailability(value) {
  ...
  if (!identity?.storageId) return { enabled: true, online: false, storageId: value.share.storageId, error: 'Backup storage identity is missing' };
...
async function localState() {
  ...
  const peerStates = await Promise.all(value.peers.map(async peer => {
    try {
      ...
      return { id: peer.id, online: remote.online !== false, ...remote };
