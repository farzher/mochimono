...
// This runs on the Agent PC, not the low-powered storage server. Two image
// workers make first-time local preview generation substantially faster while
// keeping video decoding serialized and visible requests ahead of background work.
const DEFAULT_WORKERS = Math.min(2, CPU_COUNT);
const WORKERS = Math.max(1, Math.min(4, Number(process.env.MOCHIMONO_THUMBNAIL_WORKERS) || DEFAULT_WORKERS));
...
function nextFrom(source) {
  for (const [hash, file] of source) {
...
function nextFile() {
  // Visible grid/viewer work always jumps ahead of the explicit background warmer.
  return nextFrom(queue) || nextFrom(backgroundQueue);
}
...
export function queueProviderThumbnail(file, options = {}) {
  ...
  if (!background && backgroundQueue.has(hash)) {
    ...
    queue.set(hash, { ...queued, ...file, hash });
    pump();
    return true;
  }
  if (queue.has(hash) || backgroundQueue.has(hash)) return false;

  const target = background ? backgroundQueue : queue;
  const max = background ? MAX_BACKGROUND_QUEUE : MAX_QUEUE;
  if (target.size >= max) return false;
  target.set(hash, { ...file, hash });
  pump();
  return true;
}

export function providerThumbnailQueueStatus() {
  return { active, urgent: queue.size, background: backgroundQueue.size };
}
...
export async function providerThumbnail(hash) {
  ...
  if (cached) {
    try {
      const file = await stat(cached.path);
      if (file.isFile() && file.size === cached.size) return rememberReady(hash, cached);
    } catch {}
    readyCache.delete(hash);
  }
...
export async function serveProviderThumbnail(req, res, hash) {
  ...
  if (!thumb) {
    // Never use a full local original as a grid thumbnail. If the hash is
    // available on this PC (or an attached local backup), queue a real small
    // preview and report it as not ready yet. Cloud-only hashes return false so
    // client-gateway can continue to the server's thumbnail endpoint.
    const candidate = localCandidate(hash) || await providerCandidate(hash);
    if (!candidate) return false;
    queueProviderThumbnail({
      hash,
      filename: candidate.filename || candidate.path,
      mime: candidate.mime,
      candidate
    });
