...
async function generate(file, candidate) {
  ...
  try {
    ...
    readyCache.set(file.hash, { hash: file.hash, path: pathFor(file.hash), size: result.data.length, width, height });
  } finally {
...
export async function providerThumbnail(hash) {
  ...
  try {
    ...
    const thumb = { hash, path, size: file.size, width: Number(info.width) || 0, height: Number(info.height) || 0 };
