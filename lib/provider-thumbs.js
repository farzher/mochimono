...
import { CONFIG_DIR, currentJob, settings, SYNC_INDEX_PATH } from './agent-context.js';
...
const VERSION_PATH = join(DIR, '.version');
const CACHE_PRUNE_INTERVAL = 30 * 60_000;
const EDGE = 1080;
const QUALITY = 83;
const EFFORT = 4;
...
const DRIVE_LEASE_GRACE_MS = 450;
...
const driveLeases = new Map();
let driveLeaseTimer = null;
let active = 0;
let activeVideo = 0;
...
const driveForOwner = owner => {
  const root = parse(String(owner || '')).root;
...
async function inspectThumbnail(path) {
  ...
  try {
    ...
    return { size:info.size, ...webpDimensions(header.subarray(0, bytesRead)) };
  } finally {
    await file.close();
  }
}

function scheduleCacheMaintenance(delay = CACHE_PRUNE_INTERVAL) {
  clearTimeout(cacheMaintenanceTimer);
  cacheMaintenanceTimer = setTimeout(async () => {
    try { await pruneProviderThumbnailCache(); }
    catch (error) { console.warn(`Mochimono preview cache cleanup failed: ${error?.message || error}`); }
    finally { scheduleCacheMaintenance(); }
  }, Math.max(1, delay));
  cacheMaintenanceTimer.unref?.();
}

async function prepareProviderThumbnailCache() {
  await mkdir(DIR, { recursive:true });
  const previous = String(await readFile(VERSION_PATH, 'utf8').catch(() => '')).trim();
  if (previous && previous !== String(THUMB_VERSION)) {
    await rm(DIR, { recursive:true, force:true });
    await mkdir(DIR, { recursive:true });
  }
  await writeFile(VERSION_PATH, `${THUMB_VERSION}\n`);
  scheduleCacheMaintenance(10_000);
}

async function pruneProviderThumbnailCache() {
  const index = openSyncIndex(SYNC_INDEX_PATH);
  let keep;
  try { keep = index.knownContentHashes(); }
  finally { index.close(); }

  const buckets = await readdir(DIR, { withFileTypes:true }).catch(() => []);
  let removed = 0;
  for (const bucket of buckets) {
    if (!bucket.isDirectory() || !/^[a-f0-9]{2}$/i.test(bucket.name)) continue;
    const directory = join(DIR, bucket.name);
    const entries = await readdir(directory, { withFileTypes:true }).catch(() => []);
    const removals = [];
    for (const entry of entries) {
      if (!entry.isFile()) continue;
      const name = entry.name;
      if (/^[a-f0-9]{64}\.json$/i.test(name)) {
        removals.push(rm(join(directory, name), { force:true }).then(() => { removed++; }));
        continue;
      }
      const thumb = /^([a-f0-9]{64})\.webp$/i.exec(name);
      if (thumb) {
        const hash = thumb[1].toLowerCase();
        if (!keep.has(hash) && !running.has(hash)) {
          readyCache.delete(hash);
          removals.push(rm(join(directory, name), { force:true }).then(() => { removed++; }));
        }
        continue;
      }
      const temporary = /^([a-f0-9]{64})\..+\.(?:tmp\.webp|jpg)$/i.exec(name);
      if (temporary && !running.has(temporary[1].toLowerCase())) {
        removals.push(rm(join(directory, name), { force:true }).then(() => { removed++; }));
      }
    }
    await Promise.all(removals);
  }
  if (removed) missingCache.clear();
  return removed;
}

const cacheReady = prepareProviderThumbnailCache();

function scheduleDrivePump(delay = DRIVE_LEASE_GRACE_MS) {
  if (driveLeaseTimer) return;
  driveLeaseTimer = setTimeout(() => {
    driveLeaseTimer = null;
    pump();
  }, Math.max(1, delay));
  driveLeaseTimer.unref?.();
}

function currentDriveLease(drive) {
  if (!drive) return null;
  const lease = driveLeases.get(drive);
  if (!lease) return null;
  const busy = (Number(ownerQueued.get(lease.owner)) || 0) + (Number(ownerActive.get(lease.owner)) || 0);
  if (busy) {
    lease.emptyAt = 0;
    return lease;
  }
  const now = Date.now();
  if (!lease.emptyAt) lease.emptyAt = now;
  const remaining = DRIVE_LEASE_GRACE_MS - (now - lease.emptyAt);
  if (remaining <= 0) {
    driveLeases.delete(drive);
    return null;
  }
  scheduleDrivePump(remaining);
  return lease;
}

function claimDrive(owner) {
  const drive = driveForOwner(owner);
  if (!drive || !owner) return;
  const lease = currentDriveLease(drive);
  if (!lease) driveLeases.set(drive, { owner, emptyAt:0 });
}
...
async function imageThumb(input) {
  ...
  return library(input).rotate().resize({ width:EDGE, height:EDGE, fit:'inside', withoutEnlargement:true })
    .webp({ quality:QUALITY, effort:EFFORT, smartSubsample:true }).toBuffer({ resolveWithObject:true });
}
...
async function imageFileThumb(file, candidate, frame) {
  try {
    ...
    try {
      return await decodeHeic(candidate.path, { edge:EDGE, quality:QUALITY, effort:EFFORT });
    } catch (portableError) {
      try {
...
async function generate(file, candidate) {
  ...
  try {
    ...
    await rm(join(directory, `${file.hash}.json`), { force:true }).catch(() => {});
...
export function providerThumbnailQueueStatus(owner = '') {
  ...
  owner = String(owner || '');
  const drive = owner ? driveForOwner(owner) : '';
  const lease = drive ? currentDriveLease(drive) : null;
  return {
    ...
    ownerDrive:drive,
    ownerDriveBlocked:Boolean(owner && lease?.owner && lease.owner !== owner),
    ownerDriveOwner:lease?.owner || '',
...
export async function providerThumbnail(hash) {
  ...
  try {
    ...
    return rememberReady(hash, { hash, path, size:file.size, width:file.width, height:file.height });
  } catch {
    readyCache.delete(hash);
    rememberMissing(hash);
    return null;
