...
async function generate(file, candidate) {
  ...
  try {
    ...
    await writeFile(infoPath(file.hash), `${JSON.stringify({ version: THUMB_VERSION, width: result.info.width || 0, height: result.info.height || 0 })}\n`);
  } finally {
...
export async function providerThumbnail(hash) {
  ...
  try {
    ...
    return { hash, path, size: file.size, width: Number(info.width) || 0, height: Number(info.height) || 0 };
  } catch { return null; }
}
