...
// A full local library can easily contain tens of thousands of thumbnails. Keep
// validated cache entries hot for the life of the Agent so a verification pass
// does not immediately evict them and force the next pass back onto the disk.
...
async function imageFileThumb(file, candidate, frame) {
  try {
    ...
    try {
      return await decodeHeic(candidate.path, { edge: EDGE, quality: 78, effort: 2 });
    } catch (portableError) {
      try {
...
export function refreshProviderThumbnailPolicy() {
  // Off pauses background work. Do not throw away queued hashes: switching back
  // to Idle/Max should resume the same work rather than rediscovering it.
  pump();
}
