...
export async function serveProviderThumbnail(req, res, hash) {
  ...
  if (!thumb) {
    // A direct visible thumbnail request must stay cheap. Resolve only the
    // indexed local-file map here; do not build the full cloud/backup provider
    // snapshot just to decide how to answer one image request. Backup-only
    // candidates are discovered by the normal batched thumbnail check.
