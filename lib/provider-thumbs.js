...
// Provider previews are opportunistic UI work. Keep them deliberately light so
// browsing local folders stays responsive even when many previews are missing.
...
async function generate(file, candidate) {
  ...
  try {
    ...
    // providerThumbnail() only queues generation when the destination is absent or
    // invalid. Clear any stale memory entry before replacing the disk cache so a
    // concurrent request cannot keep trusting an old path/size.
...
function nextFile() {
  // Anything requested by the visible grid/viewer always jumps ahead of the
  // explicit background warmer. At most the single preview already running can
...
export async function serveProviderThumbnail(req, res, hash) {
  ...
  // A thumbnail can be invalidated between providerThumbnail()'s stat and the
  // stream actually opening. Handle that ordinary cache race locally instead of
  // letting ReadStream emit an unhandled error that terminates the Agent.
