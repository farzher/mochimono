...
// Provider previews run on the Desktop Agent. Image resize/encode work is cheap
// enough to use several CPU cores; videos remain separately serialized below.
const DEFAULT_WORKERS = Math.max(2, Math.min(8, Math.ceil(CPU_COUNT / 2)));
const WORKERS = Math.max(1, Math.min(12, Number(process.env.MOCHIMONO_THUMBNAIL_WORKERS) || DEFAULT_WORKERS));
