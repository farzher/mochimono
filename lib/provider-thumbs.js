...
function nextFile() {
  // Visible requests always win. Background warming uses one worker so a large
  // local library can become preview-ready without competing with navigation.
  const urgent = nextFrom(queue, true);
  if (urgent) return urgent;
  if (active >= BACKGROUND_WORKERS) return null;
  return nextFrom(backgroundQueue);
}
