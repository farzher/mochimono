...
export async function serveProviderThumbnail(req, res, hash) {
  ...
  if (!thumb) {
    // Queue a local provider preview, but do not answer 404 here. Falling
    // through lets the Agent proxy an already-cached server thumbnail now,
    // while the local provider preview is generated for offline/future use.
