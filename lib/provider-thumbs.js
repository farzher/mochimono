...
import { availableParallelism, cpus, homedir, setPriority } from 'node:os';
...
const CONFIGURED_WORKERS = Number(process.env.MOCHIMONO_THUMBNAIL_WORKERS) || 0;
const MAX_WORKERS = Math.max(1, Math.min(16, CONFIGURED_WORKERS || Math.max(2, CPU_COUNT - 1)));
const INTERACTIVE_WORKERS = CONFIGURED_WORKERS
  ? MAX_WORKERS
  : Math.max(1, Math.min(MAX_WORKERS, 8, Math.ceil(CPU_COUNT / 2)));
const CONFIGURED_VIDEO_WORKERS = Number(process.env.MOCHIMONO_THUMBNAIL_VIDEO_WORKERS) || 0;
const MAX_VIDEO_WORKERS = Math.max(1, Math.min(MAX_WORKERS, CONFIGURED_VIDEO_WORKERS || Math.min(4, Math.ceil(CPU_COUNT / 2))));
const INTERACTIVE_VIDEO_WORKERS = 1;
const IDLE_AFTER_MS = 5000;
const IDLE_CPU_LIMIT = .28;
const READY_CACHE_MAX = 4096;
const MISSING_CACHE_MAX = 8192;
const MAX_QUEUE = 320;
const MAX_BACKGROUND_QUEUE = 512;
const queue = new Map();
...
function backgroundLimits() {
  if (settings.thumbnailMode === 'off' || currentJob()?.status === 'running') return { workers: 0, videos: 0 };
  if (settings.thumbnailMode === 'idle') {
    if (Date.now() - lastUserActivity < IDLE_AFTER_MS || systemCpuLoad > IDLE_CPU_LIMIT) return { workers: 0, videos: 0 };
    return { workers: 1, videos: 1 };
  }
  return { workers: Math.max(1, MAX_WORKERS - 1), videos: MAX_VIDEO_WORKERS };
}
...
function nextFile() {
  // Visible requests always win. Max mode still leaves one worker free so a
  // newly visible preview can start without waiting for the bulk queue.
...
export function refreshProviderThumbnailPolicy() {
  if (settings.thumbnailMode === 'off') backgroundQueue.clear();
  pump();
}
