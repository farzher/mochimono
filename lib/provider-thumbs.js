...
function nextFrom(source, newest = false) {
  let selected = null;
...
function nextFile() {
  // Visible requests are recency-prioritized: if the user jumps far through the
  // grid, generate where they are now rather than draining stale offscreen work.
  return nextFrom(queue, true) || nextFrom(backgroundQueue);
}
...
export function queueProviderThumbnail(file, options = {}) {
  ...
  backgroundQueue.set(hash, { ...file, hash });
  pump();
  return true;
}
