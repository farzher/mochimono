...
export function openCatalog(path) {
  ...
  db.exec(`
    ...
    -- Persistent catalog revision. Unlike sqlite total_changes(), this survives
    -- process restarts and changes only when data returned by the library catalog
    -- can materially change. Thumbnail queues/cache bookkeeping are intentionally
    -- excluded so preview generation cannot invalidate the entire library.
  ...
  // Existing catalogs predate first-class media geometry. Migrate in place, then
  // seed old metadata rows from thumbnails where possible so most libraries get
  // exact ratios immediately instead of waiting for the metadata agent to revisit
  // every file.
  const metadataColumns = new Set(db.prepare('PRAGMA table_info(media_metadata)').all().map(row => String(row.name)));
  if (!metadataColumns.has('width')) db.exec('ALTER TABLE media_metadata ADD COLUMN width INTEGER NOT NULL DEFAULT 0');
  if (!metadataColumns.has('height')) db.exec('ALTER TABLE media_metadata ADD COLUMN height INTEGER NOT NULL DEFAULT 0');
  if (!metadataColumns.has('geometry_checked')) db.exec('ALTER TABLE media_metadata ADD COLUMN geometry_checked INTEGER NOT NULL DEFAULT 0');
  db.exec(`
    UPDATE media_metadata
    SET width = CASE
          WHEN width > 0 THEN width
          ELSE COALESCE((SELECT t.width FROM thumbnails t WHERE t.object_hash = media_metadata.object_hash AND t.width > 0), 0)
        END,
        height = CASE
          WHEN height > 0 THEN height
          ELSE COALESCE((SELECT t.height FROM thumbnails t WHERE t.object_hash = media_metadata.object_hash AND t.height > 0), 0)
        END;
    UPDATE media_metadata
    SET geometry_checked = 1
    WHERE geometry_checked = 0 AND width > 0 AND height > 0;
  `);

  // Create revision triggers after migrations so merely opening an older database
  // does not churn its revision. Keep the triggers narrow: routine thumbnail,
  // integrity timestamps, drive heartbeats, and other background bookkeeping must
  // not force clients to redownload the catalog.
  db.exec(`
