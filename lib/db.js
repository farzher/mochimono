...
export function openCatalog(path) {
  ...
  if (existed) {
    ...
    const tables = Number(db.prepare("SELECT COUNT(*) AS count FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'").get()?.count) || 0;
    if (tables && version !== CATALOG_SCHEMA) {
      db.close();
      throw new Error(`Mochimono catalog schema ${version || 'legacy'} is incompatible with this pre-release build. Move or clear the MOCHIMONO_DATA directory and re-import instead of migrating old development data.`);
    }
  }
  ...
  db.exec(`
    ...
    -- Tags describe content hashes, including content that currently exists only
    -- on a client. They therefore intentionally do not reference objects.
    ...
    PRAGMA user_version = ${CATALOG_SCHEMA};
    PRAGMA optimize;
  `);
  return db;
}
