...
function defaultSignalUrl() {
  if (process.env.MOCHIMONO_SIGNAL_URL) return normalizeSignalUrl(process.env.MOCHIMONO_SIGNAL_URL);
...
export class FriendSignalingClient {
  constructor(onEvent = () => {}) {
    ...
    this.overrideBase = savedBase;
    this.base = this.overrideBase || defaultSignalUrl();
    this.identity = null;
    this.token = '';
    this.running = false;
  }
  ...
  currentBase() {
    const base = this.overrideBase || defaultSignalUrl();
    if (base !== this.base) {
      this.base = base;
      this.token = '';
    }
  ...
  async inviteBase() {
    return this.setBase(this.currentBase());
  }
  ...
  async raw(path, options = {}) {
    ...
    try {
      ...
      const configured = Boolean(process.env.MOCHIMONO_SIGNAL_URL || this.overrideBase);
