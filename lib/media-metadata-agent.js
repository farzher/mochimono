...
function dateParts(value, offset = '') {
  ...
  if (year < 1980 || year > maxYear || month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || minute > 59 || second > 59) return null;
  const calendar = new Date(Date.UTC(year, month - 1, day, hour, minute, second));
  if (calendar.getUTCFullYear() !== year || calendar.getUTCMonth() !== month - 1 || calendar.getUTCDate() !== day) return null;
  const suffix = /^[+-]\d{2}:\d{2}$/.test(offset) ? offset : '';
  const capturedAt = `${match[1]}-${match[2]}-${match[3]}T${match[4]}:${match[5]}:${match[6]}${suffix}`;
  return Number.isNaN(new Date(capturedAt).getTime()) ? null : capturedAt;
}
...
function exifFields(buffer) {
  ...
  const readIfd = offset => {
    if (visited.has(offset) || visited.size >= 16) return;
    visited.add(offset);
...
async function imageMetadata(path) {
  try {
    ...
    for (const [dateTag, offsetTag, source] of candidates) {
      ...
      if (capturedAt) return { capturedAt, source, ...dimensions };
    }
    return { capturedAt: null, source: 'none', ...dimensions };
  } catch {}
  return { capturedAt: null, source: 'none', width: 0, height: 0 };
}
...
function videoDimensionsFromText(text) {
  const value = String(text || '');
  const videoLine = value.split(/\r?\n/).find(line => /\bVideo:/.test(line)) || '';
  const match = /(?:^|[,\s])(\d{2,5})x(\d{2,5})(?=[\s,\[]|$)/.exec(videoLine);
  let width = Number(match?.[1]) || 0;
  let height = Number(match?.[2]) || 0;
  const rotationText = /rotation of\s+(-?\d+(?:\.\d+)?)\s+degrees/i.exec(value)?.[1]
    ?? /\brotate\s*:\s*(-?\d+(?:\.\d+)?)/i.exec(value)?.[1];
  const rotation = ((Number(rotationText) || 0) % 360 + 360) % 360;
  if (rotation > 45 && rotation < 135 || rotation > 225 && rotation < 315) [width, height] = [height, width];
  return {
    width: width > 0 ? Math.round(width) : 0,
    height: height > 0 ? Math.round(height) : 0
  };
...
async function videoMetadata(path) {
  ...
  return new Promise(resolvePromise => {
    ...
    const finish = () => {
      ...
      resolvePromise({ ...videoDateFromText(stderr), ...videoDimensionsFromText(stderr) });
    };
...
async function extractMetadata(path, file) {
  const extension = extname((file.sources || [])[0]?.filename || path).toLowerCase();
  const kind = String(file.mime || '').split('/')[0];
  if (kind === 'image' || IMAGE_EXTENSIONS.has(extension)) return imageMetadata(path);
  if (kind === 'video' || VIDEO_EXTENSIONS.has(extension)) return videoMetadata(path);
  return { capturedAt: null, source: 'none', width: 0, height: 0 };
}
...
async function cycle() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (!path) continue;
      const metadata = await extractMetadata(path, file);
