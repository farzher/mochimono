...
const OFFLINE_FALLBACK_RESCAN_MS = 60_000;
const OFFLINE_FULL_RESCAN_MS = 30 * 60_000;
...
function wakePump(delay = 20) {
  if (wakeTimer || stopped) return;
...
function reconcileWatchers() {
  ...
  for (const folder of settings.folders || []) {
    ...
    try {
      ...
      // Some platforms cannot recursively watch. A short fallback scan below
      // handles those roots without repeatedly walking watched Windows folders.
    }
  }
}
...
export async function startOfflineIndexService() {
  ...
  // Only roots without a reliable recursive watcher need frequent fallback scans.
  rescanTimer = setInterval(() => {
    if (lastCloudOnline) return;
    for (const folder of settings.folders || []) {
      if (!watchers.has(pathKey(folder.path))) markDirty(folder.path);
    }
  }, OFFLINE_FALLBACK_RESCAN_MS);
  // A much slower full pass protects against an OS watcher silently missing an
  // event without turning offline mode into constant disk activity.
  fullRescanTimer = setInterval(() => {
    if (lastCloudOnline) return;
    for (const folder of settings.folders || []) markDirty(folder.path);
  }, OFFLINE_FULL_RESCAN_MS);
  pumpTimer.unref?.();
  watcherTimer.unref?.();
  rescanTimer.unref?.();
  fullRescanTimer.unref?.();
...
export function stopOfflineIndexService() {
  ...
  if (fullRescanTimer) clearInterval(fullRescanTimer);
  if (wakeTimer) clearTimeout(wakeTimer);
  pumpTimer = watcherTimer = rescanTimer = fullRescanTimer = wakeTimer = null;
