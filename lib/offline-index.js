...
function queuePreview(root, path, row) {
  if (!isMediaFile(path, row.mime)) return;
...
async function indexFolder(folder, update = () => {}) {
  ...
  for await (const path of filesUnder(root, scope)) {
    ...
    if (hash) reused++;
      ...
      queuePreview(root, path, { hash, size:info.size, mime:mimeFor(path) });
    }
...
export function startOfflineIndexService() {
  if (pumpTimer) return;
  stopped = false;
  reconcileWatchers();
  ...
  watcherTimer.unref?.();
  rescanTimer.unref?.();
  setTimeout(() => pump().catch(() => {}), 100).unref?.();
}
