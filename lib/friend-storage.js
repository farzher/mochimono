...
async function friendBackupUpdate(target, update = () => {}) {
  ...
  let after = '', copied = 0, copiedBytes = 0, already = 0, skipped = 0, skippedBytes = 0;
  do {
    ...
    for (const object of page.objects || []) {
      canceled();
      const id = objectId(key, object.hash), old = prior.get(object.hash), size = Number(object.size) || 0;
      const head = old && Number(old.size) === size ? await encryptedObjectHead(target, id) : null;
      if (head && Number(head.size) === size + OVERHEAD && old.verifiedAt) {
        already++; objects.push({ ...old, hash: object.hash, size, mime: object.mime, id }); replicas.push({ hash: object.hash, verifiedAt: old.verifiedAt });
      } else {
        const needed = size + OVERHEAD;
        if (needed > budget) {
        ...
        const verifiedAt = now(); objects.push({ hash: object.hash, size, mime: object.mime, id, verifiedAt }); replicas.push({ hash: object.hash, verifiedAt }); copied++; copiedBytes += size;
      }
