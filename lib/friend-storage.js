...
const MIN_METADATA_RESERVE = 8 * 1024 * 1024;
const MAX_METADATA_RESERVE = 128 * 1024 * 1024;
...
const metadataReserve = free => Math.min(Math.max(0, Number(free) || 0), Math.min(MAX_METADATA_RESERVE, Math.max(MIN_METADATA_RESERVE, Math.floor((Number(free) || 0) * .02))));
...
async function beginHostedPut(share, id) {
  ...
  const quotaAllowed = quota ? Math.max(0, quota - baseUsed) : Infinity;
  const fs = await statfs(resolve(share.path));
  const physicalAllowed = Math.max(0, Number(fs.bavail) * Number(fs.bsize));
  const allowed = Math.min(quotaAllowed, physicalAllowed);
  ...
  const meter = new Transform({ transform(chunk, encoding, callback) {
    size += chunk.length;
    if (size > allowed) return callback(Object.assign(new Error('Friend Drive is full'), { code: 'ENOSPC' }));
...
async function saveManifest(target, manifest) { const key = keyBytes(target); return uploadEncrypted(target, manifestId(key), Readable.from([Buffer.from(JSON.stringify(manifest))]), key); }
    ...
    return await api('/api/drives/register', {
      ...
      body:{
        ...
        storage:{
          name:target.name,
          kind:'peer',
          deviceName:`${target.peerName || 'Friend'}:${target.peerId}`,
          site:target.peerName || 'Friend',
          reliability:'normal',
          remote:true,
          encrypted:true
        }
...
async function uploadFriendObject(target, object, key) { const response = await api(`/api/objects/${object.hash}`); const result = await uploadEncrypted(target, objectId(key, object.hash), Readable.fromWeb(response.body), key); if (Number(result.cipherSize) !== Number(object.size) + OVERHEAD) throw new Error(`Encrypted size mismatch for ${object.hash}`); return result; }
async function saveEncryptedCatalog(target, key) { const response = await api('/api/catalog/export'); return uploadEncrypted(target, catalogId(key), Readable.fromWeb(response.body), key); }

async function consumeFriendDeletions(target, key, prior) {
  const page = await api(`/api/protection/deletions/${encodeURIComponent(target.id)}`).catch(() => ({ deletions:[] }));
  const done = [];
  for (const item of page.deletions || []) {
    canceled();
    try {
      await request(target, 'remove', objectId(key, item.hash));
      prior.delete(item.hash);
      done.push(item.hash);
    } catch {}
  }
  if (done.length) await api(`/api/protection/deletions/${encodeURIComponent(target.id)}/ack`, { method:'POST', body:{ hashes:done } }).catch(() => {});
}
...
async function friendBackupUpdate(target, update = () => {}) {
  ...
  const storage = await request(target, 'info');
  ...
  await consumeFriendDeletions(target, key, prior);

  const objects = [...prior.values()];
  const objectHashes = new Set(objects.map(item => item.hash));
  const priorReplicas = objects.filter(item => item.verifiedAt).map(item => ({ hash:item.hash, verifiedAt:item.verifiedAt }));
  await reportReplicas(target, priorReplicas).catch(() => {});

  const reserve = metadataReserve(storage.freeBytes);
  let budget = Math.max(0, Number(storage.freeBytes) - reserve);
  let after = '', copied = 0, copiedBytes = 0, already = priorReplicas.length, skipped = 0, skippedBytes = 0;
  do {
    canceled();
    const page = await api(`/api/protection/plan/${encodeURIComponent(target.id)}?after=${encodeURIComponent(after)}&limit=500`);
    for (const object of page.objects || []) {
      canceled();
      if (objectHashes.has(object.hash)) continue;
      const size = Number(object.size) || 0, needed = size + OVERHEAD;
      if (needed > budget) {
        skipped++; skippedBytes += size;
        update({ phase:'Filling available Friend Drive space', current:object.hash.slice(0,12), copied, already, skipped, copiedBytes, capacityLimited:true, remainingCapacityBytes:budget });
        continue;
      }
      update({ phase:'Encrypting to Friend Drive', current:object.hash.slice(0,12), copied, already, skipped, copiedBytes, remainingCapacityBytes:budget });
      await uploadFriendObject(target, object, key);
      budget = Math.max(0, budget - needed);
      const verifiedAt = now();
      objects.push({ hash:object.hash, size, mime:object.mime, id:objectId(key, object.hash), verifiedAt });
      objectHashes.add(object.hash);
      await reportReplicas(target, [{ hash:object.hash, verifiedAt }]);
      copied++; copiedBytes += size;
    }
    after = page.nextAfter || '';
  } while (after);

  update({ phase:'Saving Friend Drive index', copied, already, skipped, copiedBytes, capacityLimited:Boolean(skipped), remainingCapacityBytes:budget });
  ...
  target.lastCapacityLimitedAt = skipped ? manifest.updatedAt : null;
  target.lastCapacitySkippedFiles = skipped;
  target.lastCapacitySkippedBytes = skippedBytes;
  await persist();
  return { drive:target.name, copied, already, copiedBytes, encrypted:true, capacityLimited:Boolean(skipped), skipped, skippedBytes, freeBytes:Math.max(0, budget + reserve) };
}
...
async function friendBackupVerify(target, update = () => {}) {
  ...
  for (let index = 0; index < manifest.objects.length; index++) { canceled(); const item = manifest.objects[index]; update({ phase: 'Verifying encrypted Friend Drive', current: item.hash.slice(0, 12), checked: index, total: manifest.objects.length, bad: bad.length, repaired }); let ok = false; try { ok = await verifyPlainObject(target, item, key); } catch {} if (!ok) { try { await uploadFriendObject(target, item, key); ok = true; repaired++; } catch {} } if (ok) { item.verifiedAt = now(); good.push({ hash: item.hash, verifiedAt: item.verifiedAt }); } else bad.push(item.hash); }
  ...
  target.lastVerifiedAt = now(); target.lastVerifyBad = bad.length; target.lastVerifyRepaired = repaired; target.lastVerifyCatalogHealthy = catalogHealthy; target.lastSeenAt = now(); await persist(); update({ phase: 'Done', checked: manifest.objects.length, total: manifest.objects.length, bad: bad.length, repaired, catalogHealthy });
...
async function remoteInfo(target) { try { const value = { online:true, connection:network.connectionInfo(target.peerId), ...(await request(target, 'info')) }; target.lastSeenAt = now(); return value; } catch (error) { return { online:false, connection:'offline', error:error.message }; } }
function publicTarget(target) { return { id:target.id, name:target.name, peerName:target.peerName || 'Friend', policy:target.policy || { all:true, collectionId:null }, recoveryKey:target.key, lastBackupAt:target.lastBackupAt || null, lastVerifiedAt:target.lastVerifiedAt || null, lastVerifyBad:Number(target.lastVerifyBad) || 0, lastVerifyRepaired:Number(target.lastVerifyRepaired) || 0, lastVerifyCatalogHealthy:target.lastVerifyCatalogHealthy !== false, lastRestoreAt:target.lastRestoreAt || null, lastSeenAt:target.lastSeenAt || null, lastCapacityLimitedAt:target.lastCapacityLimitedAt || null, lastCapacitySkippedFiles:Number(target.lastCapacitySkippedFiles) || 0, lastCapacitySkippedBytes:Number(target.lastCapacitySkippedBytes) || 0 }; }
...
async function createTarget(body) {
  ...
  const target = { id: randomUUID(), name: safeName(body.name) || safeName(host.name) || 'Friend Drive', peerId: host.deviceId, peerPublicKey: host.publicKey, peerName: safeName(host.deviceName) || 'Friend', shareId: host.shareId, key: suppliedKey || randomBytes(32).toString('base64url'), policy: { all: true, collectionId: null }, createdAt: now() };
  if (suppliedKey) { const manifest = await loadManifest(target); if (!manifest.driveId) throw Object.assign(new Error('Recovery key does not match an existing Friend Drive at this location'), { status: 400 }); target.id = String(manifest.driveId); target.lastBackupAt = manifest.updatedAt || null; if (config.backups.some(item => item.id === target.id)) throw Object.assign(new Error('This Friend Drive is already connected'), { status: 409 }); }
...
async function storageLocations() {
  ...
  try {
    ...
    locations.push({ id:'local-cache', type:'local', name:'Local cache', path:CONFIG_DIR, root:cacheRoot, online:true, capacityBytes:Number(fs.blocks) * Number(fs.bsize), freeBytes:Number(fs.bavail) * Number(fs.bsize), contents:'Index · thumbnails · local metadata' });
  } catch {
  ...
  for (const target of config.backups) {
    ...
    locations.push({ id:`friend:${target.id}`, type:'friend', name:target.name, online:storage.online, capacityBytes:Number(storage.capacityBytes) || 0, freeBytes:Number(storage.freeBytes) || 0, mochimonoBytes:Number(storage.usedBytes) || 0, encrypted:true, connection:storage.connection, error:storage.error || '', capacityLimited:Boolean(target.lastCapacitySkippedBytes) });
  }
  return locations;
}
...
async function handleManagement(req, res, url) {
  ...
  const contents = /^\/local\/friend-backups\/([^/]+)\/contents$/.exec(url.pathname); if (contents && req.method === 'GET') { const target = config.backups.find(item => item.id === contents[1]); if (!target) return sendJson(req, res, 404, { error: 'Friend Drive not found' }); return sendJson(req, res, 200, await friendBackupContents(target)); }
  const action = /^\/local\/friend-backups\/([^/]+)\/(update|verify|restore)$/.exec(url.pathname); if (action && req.method === 'POST') { const target = config.backups.find(item => item.id === action[1]); if (!target) return sendJson(req, res, 404, { error: 'Friend Drive not found' }); const verb = action[2], work = verb === 'update' ? friendBackupUpdate : verb === 'verify' ? friendBackupVerify : friendBackupRestore; return startJob(res, verb === 'update' ? 'backup' : verb, `Friend ${verb} ${target.name}`, update => work(target, update)); }
  return false;
}
...
server.listen(PORT, '127.0.0.1', () => console.log(`Mochimono Friend Drive management: 127.0.0.1:${PORT}`));
