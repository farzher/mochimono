...
async function storageLocations() {
  const locations = []; const state = await serverState(); if (state.online && state.stats) { const stats = state.stats; locations.push({ id: 'cloud', type: 'cloud', name: 'Cloud', online: true, capacityBytes: Number(stats.capacityBytes) || 0, freeBytes: Number(stats.freeBytes) || 0, mochimonoBytes: Number(stats.bytes) || 0 }); } else locations.push({ id: 'cloud', type: 'cloud', name: 'Cloud', online: false });
  const backupRoots = new Map(); for (const path of settings.backups || []) { const root = parse(resolve(path)).root || resolve(path); backupRoots.set(root.toLowerCase(), path); }
  const seen = new Set(), localPaths = [CONFIG_DIR, ...(settings.folders || []).map(folder => folder.path), ...(settings.browseFolders || [])]; for (const item of localPaths) { const path = resolve(String(item)), root = parse(path).root || path, key = root.toLowerCase(); if (seen.has(key) || backupRoots.has(key)) continue; seen.add(key); try { const fs = await statfs(path); locations.push({ id: `local:${key}`, type: 'local', name: `${settings.device} · ${root}`, path: root, online: true, capacityBytes: Number(fs.blocks) * Number(fs.bsize), freeBytes: Number(fs.bavail) * Number(fs.bsize) }); } catch { locations.push({ id: `local:${key}`, type: 'local', name: `${settings.device} · ${root}`, path: root, online: false }); } }
  for (const path of settings.backups || []) { const root = parse(resolve(path)).root || resolve(path), key = root.toLowerCase(); let name = basename(path) || root; try { name = (await readBackup(path)).name || name; } catch {} try { const fs = await statfs(path); locations.push({ id: `backup:${key}`, type: 'backup', name, path, online: true, capacityBytes: Number(fs.blocks) * Number(fs.bsize), freeBytes: Number(fs.bavail) * Number(fs.bsize) }); } catch { locations.push({ id: `backup:${key}`, type: 'backup', name, path, online: false }); } }
  for (const target of config.backups) { const storage = await remoteInfo(target); locations.push({ id: `friend:${target.id}`, type: 'friend', name: target.name, online: storage.online, capacityBytes: Number(storage.capacityBytes) || 0, freeBytes: Number(storage.freeBytes) || 0, mochimonoBytes: Number(storage.usedBytes) || 0, encrypted: true, connection: storage.connection, error: storage.error || '' }); }
  return locations;
}
