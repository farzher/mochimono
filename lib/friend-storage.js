...
function sameUiOrigin(req) {
  ...
  try {
    ...
    return source.hostname === host && Number(source.port || (source.protocol === 'https:' ? 443 : 80)) === AGENT_PORT;
  } catch { return false; }
}
...
function cors(req, res) {
  ...
  try {
    ...
    const hostHeader = String(req.headers.host || '');
    const host = hostHeader.startsWith('[') ? hostHeader.slice(1, hostHeader.indexOf(']')) : hostHeader.split(':')[0];
    if (source.hostname !== host || Number(source.port || (source.protocol === 'https:' ? 443 : 80)) !== AGENT_PORT) return;
...
function hostedShare(req, id) {
  ...
  return share && bearer(req) === share.token ? share : null;
}
...
async function writeHostedObject(req, res, share, id) {
  ...
  try {
    ...
    await persist();
    return sendJson(req, res, 201, { id, size, sha256 });
  } catch (error) {
    await rm(temp, { force: true }).catch(() => {});
    return sendJson(req, res, error.status || 500, { error: error.message });
  }
}
...
async function friendFetch(target, path, options = {}) {
  return fetch(`${cleanUrl(target.url)}${path}`, { ...options, headers: { authorization: `Bearer ${target.token}`, ...(options.headers || {}) }, duplex: options.body ? 'half' : undefined });
}
...
async function uploadEncrypted(target, id, source, key) {
  const state = {};
  const result = await friendJson(target, `/objects/${id}`, { method: 'PUT', headers: { 'content-type': 'application/octet-stream' }, body: encryptedReadable(source, key, state) });
...
async function encryptedObjectHead(target, id) {
  ...
  return response?.ok ? { size: Number(response.headers.get('content-length')) || 0 } : null;
}
...
async function loadManifest(target) {
  ...
  try {
    const value = JSON.parse((await decryptBuffer(target, manifestId(key), key)).toString('utf8'));
...
async function saveManifest(target, manifest) {
  const key = keyBytes(target);
  await uploadEncrypted(target, manifestId(key), Readable.from([Buffer.from(JSON.stringify(manifest))]), key);
}

async function registerFriendBackup(target) {
  try { return await api('/api/drives/register', { method: 'POST', body: { id: target.id, name: target.name, policy: target.policy || { all: true, collectionId: null } } }); }
  catch { return null; }
}

async function reportReplicas(target, replicas) {
  for (let offset = 0; offset < replicas.length; offset += 1000) await api(`/api/drives/${encodeURIComponent(target.id)}/replicas`, { method: 'POST', body: { replicas: replicas.slice(offset, offset + 1000) } });
}

async function removeReplicas(target, hashes) {
  for (let offset = 0; offset < hashes.length; offset += 1000) await api(`/api/drives/${encodeURIComponent(target.id)}/replicas/remove`, { method: 'POST', body: { hashes: hashes.slice(offset, offset + 1000) } });
}
...
async function friendBackupUpdate(target, update = () => {}) {
  ...
  do {
    ...
    for (const object of page.objects || []) {
      ...
      if (head && Number(head.size) === Number(object.size) + OVERHEAD && old.verifiedAt) {
        already++;
...
async function verifyPlainObject(target, item, key) {
  ...
  const digest = createHash('sha256');
  let size = 0;
  for await (const chunk of decryptedIterable(Readable.fromWeb(response.body), key)) { digest.update(chunk); size += chunk.length; }
...
async function friendBackupVerify(target, update = () => {}) {
  ...
  for (let index = 0; index < manifest.objects.length; index++) {
    ...
    if (!ok) {
      try { await uploadFriendObject(target, item, key); ok = true; repaired++; } catch {}
    }
    if (ok) { item.verifiedAt = now(); good.push({ hash: item.hash, verifiedAt: item.verifiedAt }); }
...
async function restoreSources(db, ignored) {
  ...
  const sourceNames = db.prepare(`SELECT DISTINCT i.source_name AS sourceName FROM sources s JOIN imports i ON i.id=s.import_id JOIN friend_inventory.objects b ON b.hash=s.object_hash ORDER BY lower(i.source_name)`).all().map(item => String(item.sourceName));
  for (const sourceName of sourceNames) {
    ...
    const rows = db.prepare(`SELECT s.object_hash AS hash,s.original_path AS path,s.filename,s.mtime FROM sources s JOIN imports i ON i.id=s.import_id JOIN friend_inventory.objects b ON b.hash=s.object_hash WHERE i.source_name=? ORDER BY s.original_path`).all(sourceName).filter(row => !ignored.has(row.hash));
    for (let offset = 0; offset < rows.length; offset += 1000) await api('/api/sources', { method: 'POST', body: { importId: importIds.get(sourceName), sources: rows.slice(offset, offset + 1000) } });
  }
  const roots = db.prepare(`SELECT i.source_name AS sourceName,COALESCE(MAX(ir.device_name),'') AS deviceName,COALESCE(MAX(ir.root_path),'') AS rootPath FROM imports i JOIN sources s ON s.import_id=i.id JOIN friend_inventory.objects b ON b.hash=s.object_hash LEFT JOIN import_roots ir ON ir.import_id=i.id GROUP BY i.source_name`).all().filter(item => item.deviceName || item.rootPath);
...
async function restoreMetadata(db) {
  const rows = db.prepare(`SELECT mm.object_hash AS hash,mm.captured_at AS capturedAt,mm.source FROM media_metadata mm JOIN friend_inventory.objects b ON b.hash=mm.object_hash`).all();
  for (let offset = 0; offset < rows.length; offset += 20) await Promise.all(rows.slice(offset, offset + 20).map(row => api(`/api/media-metadata/${row.hash}`, { method: 'POST', body: { capturedAt: row.capturedAt, source: row.source } })));
}
...
async function friendBackupRestore(target, update = () => {}) {
  ...
  try {
    ...
    for (let index = 0; index < manifest.objects.length; index++) {
      ...
      if (missing.has(item.hash)) {
        const response = await encryptedObjectResponse(target, item.id || objectId(key, item.hash));
        await api(`/api/objects/${item.hash}`, { method: 'PUT', headers: { 'content-length': String(item.size), 'x-mochimono-mime': String(item.mime || 'application/octet-stream') }, body: Readable.from(decryptedIterable(Readable.fromWeb(response.body), key)) });
        restored++;
  ...
  } finally { await Promise.all([rm(catalogPath, { force: true }), rm(inventoryPath, { force: true })]); }
}

async function friendBackupContents(target) {
  const manifest = await loadManifest(target);
  return { name: target.name, count: manifest.objects.length, bytes: manifest.objects.reduce((sum, item) => sum + Number(item.size || 0), 0), updatedAt: manifest.updatedAt || target.lastBackupAt || null, encrypted: true };
}
...
async function createTarget(body) {
  const suppliedKey = normalizedRecoveryKey(body.recoveryKey);
  if (body.recoveryKey && !suppliedKey) throw Object.assign(new Error('Recovery key is invalid'), { status: 400 });
  const target = {
    id: randomUUID(),
    name: safeName(body.name) || 'Friend backup',
    url: cleanUrl(body.url),
    token: String(body.token || '').trim(),
    key: suppliedKey || randomBytes(32).toString('base64url'),
  ...
  if (suppliedKey) {
    const manifest = await loadManifest(target);
    if (!manifest.driveId) throw Object.assign(new Error('Recovery key does not match an existing friend backup at this location'), { status: 400 });
    target.id = String(manifest.driveId);
    target.lastBackupAt = manifest.updatedAt || null;
    if (config.backups.some(item => item.id === target.id)) throw Object.assign(new Error('This friend backup is already connected'), { status: 409 });
  }
...
async function createShare(body) {
  ...
  const share = { id, name: safeName(body.name) || basename(path) || 'Friend storage', path, token, quotaBytes: Math.max(0, Number(body.quotaBytes) || 0), usedBytes: 0, createdAt: now() };
...
async function storageLocations() {
  ...
  const seen = new Set();
  const localPaths = [CONFIG_DIR, ...(settings.folders || []).map(folder => folder.path), ...(settings.browseFolders || [])];
  for (const item of localPaths) {
    ...
    try {
      ...
      locations.push({ id: `local:${key}`, type: 'local', name: `${settings.device} · ${root}`, path: root, online: true, capacityBytes: Number(fs.blocks) * Number(fs.bsize), freeBytes: Number(fs.bavail) * Number(fs.bsize) });
    } catch { locations.push({ id: `local:${key}`, type: 'local', name: `${settings.device} · ${root}`, path: root, online: false }); }
  }
  for (const path of settings.backups || []) {
    const root = parse(resolve(path)).root || resolve(path);
    const key = root.toLowerCase();
  ...
  for (const target of config.backups) {
    ...
    locations.push({ id: `friend:${target.id}`, type: 'friend', name: target.name, online: storage.online, capacityBytes: Number(storage.capacityBytes) || 0, freeBytes: Number(storage.freeBytes) || 0, mochimonoBytes: Number(storage.usedBytes) || 0, encrypted: true, error: storage.error || '' });
  }
  return locations;
}
...
async function handleManagement(req, res, url) {
  cors(req, res);
  if (!sameUiOrigin(req)) return sendJson(req, res, 403, { error: 'Friend storage management is local to this Mochimono Agent' });
  ...
  if (targetDelete && req.method === 'DELETE') {
    const target = config.backups.find(item => item.id === targetDelete[1]);
    if (target) {
      try {
        const manifest = await loadManifest(target);
        await removeReplicas(target, manifest.objects.map(item => item.hash));
      } catch {}
    }
