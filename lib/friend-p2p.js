...
export class FriendPeerNetwork extends CoreFriendPeerNetwork {
  ...
  async createInvite(share) {
    const signalUrl = this.signal.currentBase();
    try {
      const invite = await super.createInvite(share);
      await this.signal.pinBase(signalUrl);
      return { ...invite, code: encodeInvite(signalUrl, invite.code) };
    } catch (error) {
      this.signal.resetSelectedBase();
      throw error;
    }
  }
  ...
  async joinInvite(code) {
    ...
    this.signal.selectBase(invite.signalUrl);
    try {
      const peer = await super.joinInvite(invite.secretCode);
      await this.signal.pinBase(invite.signalUrl);
