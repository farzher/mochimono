...
import { homedir, availableParallelism, platform } from 'node:os';
...
async function localSource(settings, file) {
  const folder = settings.folders.find(item => item.importId === Number(file.importId));
  if (!folder) return null;
  const path = safeLocalPath(folder.path, file.originalPath);
  if (!path) return null;
  let info;
  try { info = await stat(path); } catch { return null; }
  if (!info.isFile() || Number(info.size) !== Number(file.size)) return null;
  if (file.mtime) {
    const expected = Date.parse(file.mtime);
...
function runFfmpeg(path, kind) {
  return new Promise(async (resolvePromise, reject) => {
    ...
    const args = [
      '-hide_banner', '-loglevel', 'info', '-threads', '1',
      '-i', path,
      '-map', '0:v:0', '-an', '-sn', '-dn',
      '-vf', filter,
      '-frames:v', '1',
...
function queueFile(file) {
  if (!file?.hash || queued.has(file.hash) || running.has(file.hash)) return;
  if ((failedUntil.get(file.hash) || 0) > Date.now()) return;
  if (!kindFor(file)) return;
  queued.set(file.hash, file);
}
...
async function processFile(file) {
  ...
  try {
    const result = await runFfmpeg(path, kind);
    await uploadThumbnail(settings, file, result);
    failedUntil.delete(file.hash);
    lastDependencyError = '';
  } catch (error) {
    const dependencyError = error.code === 'ENOENT' || /ffmpeg/i.test(error.message) && /not found|enoent/i.test(error.message);
    if (dependencyError) {
      if (lastDependencyError !== error.message) {
        lastDependencyError = error.message;
        console.warn('Mochimono previews: FFmpeg is unavailable. Run npm install or set FFMPEG_PATH.');
      }
...
function pump() {
  while (!stopped && active < WORKERS && queued.size) {
    const [hash, file] = queued.entries().next().value;
    queued.delete(hash);
    running.add(hash);
    active++;
...
async function discover() {
  ...
  const result = await api(settings, `/api/thumbs/missing?imports=${encodeURIComponent(imports)}&limit=250`);
  for (const file of result.files || []) queueFile(file);
  pump();
}
...
async function loop() {
  ...
  if (!stopped) loopTimer = setTimeout(loop, queued.size || active ? 1000 : 4000);
}
...
export function thumbnailAgentStatus() {
  return { workers: WORKERS, queued: queued.size, active, failed: [...failedUntil.values()].filter(time => time > Date.now()).length };
}
