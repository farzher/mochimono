...
async function validDirectCandidate(file) {
  ...
  if (!sameMtime(info.mtimeMs, file.mtime)) return null;
  return path;
}
...
async function validLocalCandidate(settings, file, source) {
  ...
  try { info = await stat(path); } catch { return null; }
  if (!info.isFile() || Number(info.size) !== Number(file.size)) return null;
  if (!sameMtime(info.mtimeMs, source.mtime)) return null;
  return path;
}
...
async function localSource(settings, file) {
  ...
  for (const source of candidates) {
    ...
    if (path) return { path, source, direct: false };
  }
  return null;
}
...
async function encodeFrame(result) {
  const sharp = await sharpLibrary();
  const { data, info } = await sharp(result.blob)
    .webp({ quality: 78, effort: 2, smartSubsample: true })
    .toBuffer({ resolveWithObject: true });
  if (!data.length) throw new Error('Video preview is empty');
  if (data.length > MAX_OUTPUT) throw new Error('Generated preview is too large');
  return {
    blob: data,
    width: info.width || 0,
...
async function previewIsBlank(blob) {
  try {
    const sharp = await sharpLibrary();
    const stats = await sharp(blob).stats();
...
function parseDuration(stderr) {
  ...
  return duration ? Number(duration[1]) * 3600 + Number(duration[2]) * 60 + Number(duration[3]) : null;
}

function ffmpegError(stderr, code) {
  const lines = String(stderr || '').split(/\r?\n/).map(line => line.trim()).filter(Boolean);
  const generic = /^(Error opening (input|output) files?|Error parsing options for output file|Conversion failed)!?:?/i;
  const useful = [...lines].reverse().find(line =>
    !generic.test(line) && /invalid|error|failed|unsupported|could not|not found|no such file|moov|decoder|corrupt|damaged/i.test(line)
  );
  return new Error(useful || lines.at(-1) || `FFmpeg exited with ${code}`);
}

function deterministicMediaFailure(message) {
  return /invalid data found when processing input|moov atom not found|could not find codec parameters|decoder .* not found|unsupported codec|invalid nal|corrupt|damaged/i.test(String(message || ''));
}
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    child.stdout.on('data', chunk => {
      ...
      if (bytes > MAX_OUTPUT) {
        ...
        settled = true;
        clearTimeout(timer);
        reject(new Error('Generated preview frame is too large'));
        return;
      }
      chunks.push(chunk);
    });
    ...
    child.on('close', code => {
      ...
      settled = true;
      clearTimeout(timer);
      if (code !== 0 || !bytes) return reject(ffmpegError(stderr, code));
      resolvePromise({ blob: Buffer.concat(chunks), duration: parseDuration(stderr) });
    });
  });
}

async function ffmpegPreview(path, kind, seekSeconds = 0) {
  return encodeFrame(await runFfmpeg(path, kind, seekSeconds));
}

async function runVideoPreview(path) {
  let first;
  try { first = await ffmpegPreview(path, 'video', .5); }
  catch { first = await ffmpegPreview(path, 'video', 0); }
  if (!await previewIsBlank(first.blob)) return first;

  for (const seek of [2, 8]) {
    try {
      const later = await ffmpegPreview(path, 'video', seek);
...
async function generatePreview(path, kind) {
  ...
  try { return await runSharp(path); }
    ...
    if (/cannot find package|module not found|ERR_MODULE_NOT_FOUND/i.test(`${error.code || ''} ${error.message}`)) {
      if (lastDependencyError !== 'sharp') {
        lastDependencyError = 'sharp';
        console.warn('Mochimono previews: Sharp is unavailable. Run npm install.');
      }
      throw error;
    }
    return ffmpegPreview(path, kind);
  }
}
...
function takeEligible(queue) {
  for (const [hash, file] of queue) {
    if (kindFor(file) === 'video' && activeVideo >= VIDEO_WORKERS) continue;
    queue.delete(hash);
    return file;
  }
  return null;
...
function nextFile() {
  return takeEligible(urgentQueue) || takeEligible(priorityQueue) || takeEligible(backgroundQueue);
}
...
async function processFile(file) {
  ...
  try {
    const result = await generatePreview(local.path, kind);
    const stillValid = local.direct
      ? await validDirectCandidate(file)
      : await validLocalCandidate(settings, file, local.source);
    if (stillValid !== local.path) return;
    await uploadThumbnail(settings, file, result);
    failedUntil.delete(file.hash);
    warnedFailures.delete(file.hash);
    if (lastDependencyError !== 'sharp') lastDependencyError = '';
  } catch (error) {
    ...
    const terminal = kind === 'video' && deterministicMediaFailure(error.message);
    const label = file.filename || file.hash.slice(0, 12);
    const warning = `${terminal ? 'unavailable' : 'failed'}:${error.message}`;
    if (warnedFailures.get(file.hash) !== warning) {
      warnedFailures.set(file.hash, warning);
      console.warn(`Mochimono preview ${terminal ? 'unavailable' : 'failed'} for ${label} [${file.hash.slice(0, 12)}]: ${error.message}`);
    }
...
function pump() {
  while (!stopped && active < WORKERS && (urgentQueue.size || priorityQueue.size || backgroundQueue.size)) {
    const file = nextFile();
    if (!file) break;
