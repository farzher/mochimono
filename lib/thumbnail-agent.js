...
import { availableParallelism, platform, setPriority } from 'node:os';
import { extname, join, resolve, sep } from 'node:path';
import { Readable } from 'node:stream';
import { pipeline } from 'node:stream/promises';
import { spawn } from 'node:child_process';
import { CONFIG_DIR, settings as agentSettings } from './agent-context.js';
import { backgroundWorkAllowed, backgroundWorkStatus } from './background-work.js';
import { decodeHeic } from './heic.js';

const TMP_DIR = join(CONFIG_DIR, 'tmp');
const THUMB_VERSION = 4;
const THUMB_EDGE = 1280;
const THUMB_QUALITY = 83;
...
const typescriptDeclaration = name => /\.d\.(?:mts|cts|ts)$/i.test(String(name || ''));

function kindFor(file) {
  const filename = String(file?.filename || '');
  if (typescriptDeclaration(filename)) return '';
...
function mimeFor(file) {
  if (typescriptDeclaration(file?.filename)) return 'text/typescript';
...
function isHeic(file, path = '') {
  const mime = String(file?.mime || '').toLowerCase();
  return mime === 'image/heic' || mime === 'image/heif' || HEIF_EXTENSIONS.has(extname(file?.filename || path || '').toLowerCase());
}
...
async function heicPreview(path) {
  const result = await decodeHeic(path, { edge: THUMB_EDGE, quality: THUMB_QUALITY, effort: THUMB_EFFORT });
  if (!result.data.length) throw new Error('HEIC preview is empty');
  if (result.data.length > MAX_OUTPUT) throw new Error('HEIC preview is too large');
  return { blob: result.data, width: result.info.width || 0, height: result.info.height || 0, duration: null };
}
...
async function imagePreview(path, file = {}) {
  try { return await sharpPreview(path); }
    ...
    if (isHeic(file, path)) {
      try { return await heicPreview(path); }
      catch {}
    }
...
async function remoteImage(settings, file) {
  ...
  try {
    ...
    return await imagePreview(path, file);
  } finally {
...
async function generatePreview(settings, file, local) {
  ...
  if (local) return kind === 'video' ? videoPreview(local.path) : imagePreview(local.path, file);
...
function queueFile(file) {
  ...
  if (file.requestedAt) {
    ...
    const existing = urgentQueue.delete(file.hash);
    if (!existing && urgentQueue.size >= MAX_URGENT_QUEUE) {
...
function nextFrom(queue, videoLimit = VIDEO_WORKERS, newest = false) {
  let selected = null;
...
function nextFile() {
  const urgent = nextFrom(urgentQueue, VIDEO_WORKERS, true) || nextFrom(priorityQueue);
  if (urgent) return urgent;
  if (!backgroundWorkAllowed()) return null;
...
async function discover() {
  ...
  const imports = settings.folders.map(item => item.importId).filter(Boolean).join(',');
  if (!imports) return;

  // Priority=1 is driven by visible/on-demand requests, so keep this tiny poll
  // responsive even when background work is paused.
...
export function thumbnailAgentStatus() {
  ...
  return {
    ...
    active,
    activeVideo,
    waitingForIdle: Boolean(backgroundQueue.size && !policy.allowed),
