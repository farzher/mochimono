...
async function inspectPreview(result) {
  try {
    const sharp = await sharpLibrary();
    const metadata = await sharp(result.blob).metadata();
    return {
      ...result,
      width: metadata.width || result.width || 0,
      height: metadata.height || result.height || 0
    };
  } catch {
    return result;
  }
...
async function runFfmpeg(path, kind, seekSeconds = 0) {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const args = [
      ...
      '-vf', scale,
      '-frames:v', '1',
      '-c:v', 'libwebp', '-threads', '1', '-lossless', '0', '-quality', '78', '-compression_level', '3',
      '-f', 'image2pipe', 'pipe:1'
    ];
    ...
    child.on('close', code => {
      ...
      resolvePromise({ blob: Buffer.concat(chunks), width: 0, height: 0, duration: parseDuration(stderr) });
    });
  });
}

async function ffmpegPreview(path, kind, seekSeconds = 0) {
  return inspectPreview(await runFfmpeg(path, kind, seekSeconds));
}
  ...
  try { return await runSharp(path); }
    ...
    if (/cannot find package|module not found|ERR_MODULE_NOT_FOUND/i.test(`${error.code || ''} ${error.message}`)) {
      if (lastDependencyError !== 'sharp') {
        ...
        console.warn('Mochimono previews: Sharp is unavailable; images are using the slower FFmpeg fallback. Run npm install.');
      }
    }
