...
function ffmpegError(stderr, code) {
  ...
  const generic = /^(Error opening (input|output) files|Conversion failed)!?:?/i;
  const useful = [...lines].reverse().find(line =>
    !generic.test(line) && /invalid|error|failed|unsupported|could not|not found|moov|decoder|corrupt|damaged/i.test(line)
  );
