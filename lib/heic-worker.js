...
const MAX_PORTABLE_PIXELS = 24 * 1024 * 1024;
sharp.concurrency(1);
...
async function sourceDimensions(input) {
  try { return displayDimensions(await sharp(input).metadata()); }
  catch { return { width:0, height:0 }; }
}

function withSourceDimensions(result, source) {
  if (!result?.info) result.info = {};
  if (Number(source?.width) > 0) result.info.sourceWidth = Number(source.width);
  if (Number(source?.height) > 0) result.info.sourceHeight = Number(source.height);
  return result;
}
...
async function portableDecode(input, job) {
  ...
  if (width * height > MAX_PORTABLE_PIXELS) throw new Error('Portable HEIC decode would use too much memory');
  const pixels = Buffer.from(decoded.data.buffer, decoded.data.byteOffset, decoded.data.byteLength);
  return withSourceDimensions(
    await encode(sharp(pixels, { raw:{ width, height, channels:4 } }), job),
    { width, height }
  );
}
...
async function render(job) {
  ...
  const source = await sourceDimensions(input);
  const errors = [];
  try { return withSourceDimensions(await sharpDecode(input, job), source); }
  catch (error) { errors.push(`sharp: ${error?.message || error}`); }
  try { return withSourceDimensions(await ffmpegDecode(job.path, job), source); }
  catch (error) { errors.push(`ffmpeg: ${error?.message || error}`); }
  if (job.portable !== false) {
    if (source.width && source.height && source.width * source.height > MAX_PORTABLE_PIXELS) {
      errors.push('portable: skipped oversized raw decode');
    } else {
      try { return withSourceDimensions(await portableDecode(input, job), source); }
  ...
  try {
    ...
    parentPort.postMessage({
      ...
      info:{
        ...
        sourceWidth:Number(result.info.sourceWidth) || 0,
        sourceHeight:Number(result.info.sourceHeight) || 0
      }
    });
  } catch (error) {
