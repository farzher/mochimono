...
const PROVIDER_CATALOG_VERSION = 2;
let cache = null;
...
async function buildSnapshot(roots) {
  ...
  for (const row of local.rows) {
    ...
    const modified = new Date(Number(row.mtimeMs) || timeline.ms).toISOString();
...
async function buildProviders() {
  for (;;) {
    ...
    data.version = key;
    cache = { generation, key, data, checkedAt: Date.now() };
    return data;
  }
}
...
export async function clientProviders() {
  ...
  const pending = buildProviders();
  buildPromise = pending;
  try {
    return await pending;
  } finally {
    if (buildPromise === pending) buildPromise = null;
  }
}

export async function clientProviderVersion() {
  ...
  for (;;) {
    ...
    const key = await versionKey(roots);
    if (generation !== cacheGeneration) continue;
    if (cache?.generation === generation && cache.key === key) cache.checkedAt = Date.now();
    return key;
  }
}
...
export async function handleClientProviderApi(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/catalog/version') {
    json(res, 200, { version: await clientProviderVersion() });
    return true;
  }
