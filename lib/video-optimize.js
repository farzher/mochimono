...
const normalizedOptions = raw => ({
  ...
  maxEdge:[0,640,854,1280,1920,2560].includes(Number(raw?.maxEdge)) ? Number(raw.maxEdge) : 0,
...
function ratePlan(session) {
  ...
  // The compressor should compress by default. CQ remains the visual-quality
  // guardrail while this target gives each quality setting a useful size range.
  const targetVideoKbps = Math.max(80, Math.round(sourceVideo * Math.min(.97, qualityRatio * complexityScale)));
  const maxVideoKbps = Math.max(targetVideoKbps + 48, Math.round(targetVideoKbps * 1.5));
  return {
