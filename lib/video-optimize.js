...
// 72 is the default UI quality. Map it to about CQ/CRF 30: a strong AV1
// archival starting point without silently asking the encoder for near-lossless
// output. Lower codec Q means higher quality.
const qFor = quality => Math.max(20, Math.min(52, Math.round(52 - clamp(quality, 1, 100) * .30)));
const normalizedOptions = raw => ({
  ...
  maxEdge:[0,1280,1920,2560,3840].includes(Number(raw?.maxEdge)) ? Number(raw.maxEdge) : 0,
  fps:[0,30,60].includes(Number(raw?.fps)) ? Number(raw.fps) : 0
});
...
function ratePlan(session) {
  ...
  const sourceAudio = session.meta.audio ? Math.max(48, Number(session.meta.audioKbps) || Math.min(128, sourceTotal * .08)) : 0;
  const audioKbps = session.meta.audio ? Math.max(48, Math.min(128, Math.round(sourceAudio))) : 0;
  const sourceVideo = Math.max(160, sourceTotal - sourceAudio);
  const pixelRatio = Math.max(.05, (session.target.width * session.target.height) / Math.max(1, session.meta.width * session.meta.height));
  ...
  const qualityRatio = .35 + .65 * (session.options.quality / 100);
  // The compressor should compress by default. Keep a little headroom below the
  // source bitrate even at Quality 100; CQ remains the visual-quality guardrail.
  const targetVideoKbps = Math.max(120, Math.round(sourceVideo * Math.min(.97, qualityRatio * complexityScale)));
  const maxVideoKbps = Math.max(targetVideoKbps + 64, Math.round(targetVideoKbps * 1.5));
  return {
...
function outputArgs(session) {
  const fps = session.targetFps || session.meta.fps || 30;
  const gop = Math.max(24, Math.round(fps * 2));
  const audio = session.meta.audio
    ? ['-c:a','libopus','-b:a',`${session.rate.audioKbps}k`,'-vbr','on','-compression_level','10']
    : ['-an'];
  return ['-g',String(gop),...audio,'-map_metadata','0','-f','webm','-progress','pipe:1','-nostats'];
}
...
function args(session, out, plan, sample, conservative = false) {
  ...
  if (session.meta.audio) result.push('-map','0:a:0?');
  result.push('-sn');
  ...
  return result.concat(encoderArgs(plan, session, conservative), outputArgs(session), [out]);
}
...
async function keepPath(path) {
  const extension = extname(path);
  const stem = path.slice(0, -extension.length);
  let target = extension.toLowerCase() === '.webm' ? `${stem}.compressed.webm` : `${stem}.webm`;
  if (!await exists(target)) return target;
  for (let index=2; index<1000; index++) {
    target = `${stem}.compressed-${index}.webm`;
    if (!await exists(target)) return target;
  }
...
async function place(session, out, mode) {
  ...
  const final = mode === 'keep' ? await keepPath(source) : extension.toLowerCase() === '.webm' ? source : `${stem}.webm`;
...
async function commit(session, mode) {
  const out = join(DIR, `${session.id}-full.webm`);
  try {
