...
function ratePlan(session) {
  ...
  // Auto remains the existing source-aware calculation. A custom target is a
  // deliberate rate-control override and is capped at the detected source video
  // bitrate so the control's useful range is 10 kbps -> source bitrate.
  const autoVideoKbps = Math.max(10, Math.round(sourceVideo * Math.min(.97, qualityRatio * complexityScale)));
  const requestedVideoKbps = Number(session.options.videoBitrateKbps) || 0;
  const targetVideoKbps = requestedVideoKbps
    ? Math.max(10, Math.min(Math.round(sourceVideo), Math.round(requestedVideoKbps)))
    : autoVideoKbps;
...
function encoderArgs(plan, session, conservative = false) {
  ...
  const customBitrate = Number(session.options.videoBitrateKbps) >= 10;
  if (plan.kind === 'nvenc') {
  ...
  if (plan.kind === 'svt') {
    ...
    if (customBitrate) args.push('-b:v',`${rate.targetVideoKbps}k`,'-maxrate',`${rate.maxVideoKbps}k`,'-bufsize',`${rate.bufferKbits}k`);
    args.push('-preset',String(preset),'-pix_fmt','yuv420p10le');
    if (!customBitrate && /svtav1-params/i.test(plan.help || '')) args.push('-svtav1-params',`mbr=${rate.maxVideoKbps}`);
    return args;
  }

  const cpu = Math.max(1, Math.min(8, Math.round(8 - session.options.effort / 9 * 7)));
  const bitrate = customBitrate ? `${rate.targetVideoKbps}k` : '0';
  return ['-c:v','libaom-av1','-crf',String(quality),'-b:v',bitrate,'-maxrate',`${rate.maxVideoKbps}k`,'-bufsize',`${rate.bufferKbits}k`,'-cpu-used',String(cpu),'-row-mt','1','-pix_fmt','yuv420p10le'];
}
...
async function createSession(hashId, raw, at) {
  ...
  if (!candidate) throw Object.assign(Error('Squishing videos currently requires a local copy'), { status:404 });
...
const get = id => {
  ...
  if (!session) throw Object.assign(Error('Squish preview expired'), { status:404 });
  session.updatedAt = Date.now();
  return session;
};
...
const pub = session => ({
  ...
  rate:session.rate ? {
    ...
    autoVideoKbps:session.rate.autoVideoKbps,
    sourceVideoKbps:session.rate.sourceVideoKbps,
...
async function serve(req, res, url) {
  ...
  if (!info?.isFile()) throw Object.assign(Error('Squished preview is unavailable'), { status:404 });
...
async function place(session, out, mode) {
  ...
  if (await hash(stage) !== digest) {
    await unlink(stage).catch(() => {});
    throw Error('Squished copy failed verification');
  }
...
function startCommit(session, mode) {
  if (session.status !== 'ready') throw Object.assign(Error('Squish preview is not ready'), { status:409 });
  if (!['keep','replace'].includes(mode)) throw Object.assign(Error('Choose Save squished or replace original'), { status:400 });
