...
const LIVE_INDEX_MAX_BYTES = 128 * 1024 * 1024;
...
async function hashWatchedFile(path) {
  const hash = createHash('sha256');
  for await (const chunk of createReadStream(path, { highWaterMark: FILE_STREAM_BUFFER_BYTES })) hash.update(chunk);
  return hash.digest('hex');
}
...
function scheduleLocalIndex(folder, delay = LIVE_INDEX_DELAY_MS) {
  ...
  const timer = setTimeout(() => {
    ...
    void indexWatchedChanges(folder);
  }, Math.max(0, delay));
  timer.unref?.();
...
async function indexWatchedChanges(folder) {
  ...
  try {
    for (const relativePath of pending) {
      ...
      if (Number(file.size) > LIVE_INDEX_MAX_BYTES) continue;
      ...
      if (previous && Number(previous.size) === Number(file.size) && Number(previous.mtimeMs) === mtimeMs && previous.hash) {
        publishWatchedRecord(folder, {
          relative:rel, size:file.size, mtimeMs, hash:String(previous.hash)
        });
        continue;
      }

      let hash;
      try { hash = await hashWatchedFile(path); }
      catch { continue; }
      const latest = await stat(path).catch(() => null);
      if (!latest?.isFile()) continue;
      if (Number(latest.size) !== Number(file.size) || Math.trunc(latest.mtimeMs) !== mtimeMs) {
        queueLocalIndex(folder, rel, 500);
        continue;
      }
      syncIndex.save(rootKey, cachePath, file.size, mtimeMs, hash);
...
function createUploadPool(update, root) {
  ...
  function pump() {
    while (active < uploadWorkers && queue.length) {
...
async function syncFiles(folderPath, update, importId = null, scope = 'media') {
  ...
  for await (const path of filesUnder(root, selectedScope)) {
    try {
      await waitForBackgroundWork();
...
async function syncChangedFiles(folder, update) {
  ...
  const dirty = consumeDirty(root);
  if (dirty.all || !folder.importId) return syncFiles(root, update, folder.importId, selectedScope);
  ...
  for (const relativePath of dirty.paths) {
    ...
    try {
      ...
      if (previous && previous.hash && Number(previous.size) === Number(record.size) && Number(previous.mtimeMs) === Number(record.mtimeMs)) {
        record.hash = String(previous.hash);
        readyRecords.push(record);
      } else hashRecords.push(record);
    } catch {
  ...
  if (needsFullScan) return syncFiles(root, update, folder.importId, selectedScope);
...
async function syncFolder(folder, update) {
  ...
  try {
    ...
    const result = incremental ? await syncChangedFiles(folder, update) : await syncFiles(folder.path, update, folder.importId, folder.scope);
...
export function pumpSyncs() {
  ...
  beginJob('sync', `Sync ${basename(folder.path) || folder.path}`, update => syncFolder(folder, update), { background: !foreground });
}
