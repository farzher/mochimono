...
export async function backupVerify(path, update = () => {}) {
  ...
  update({ phase: 'Done', checked: rows.length, total: rows.length, bad: badCount, repaired, primaryRepaired });
  return { drive: meta.name, checked: rows.length, healthy: rows.length - badCount, bad: badCount, repaired, primaryRepaired };
}
