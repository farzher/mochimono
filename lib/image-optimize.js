...
function normalizeEffort(value, format) {
  const max = format === 'avif' ? 9 : 6;
  if (value === 'max') return max;
  if (value === 'normal' || value === '' || value == null) return 4;
  const parsed = Number(value);
  return Number.isFinite(parsed) ? Math.max(0, Math.min(max, Math.round(parsed))) : 4;
}
...
function normalizeOptions(raw = {}) {
  ...
  return {
    ...
    content,
    effort: normalizeEffort(raw.effort, format),
...
async function encodeCandidate(session, spec, { effort = session.options.effort, id = '', record = true } = {}) {
  ...
  if (Number(encoded.width) !== Number(session.target.width) || Number(encoded.height) !== Number(session.target.height)) {
    throw new Error('Squished image dimensions changed unexpectedly');
  }
  const item = {
    id: candidateId,
    path,
...
async function encodeSession(session) {
  try {
    ...
    if (metadata.depth && metadata.depth !== 'uchar') throw Object.assign(new Error("16-bit/HDR images can't be squished safely yet"), { status: 415 });
    if (String(metadata.space || '').toLowerCase() === 'cmyk') throw Object.assign(new Error("CMYK images can't be squished safely yet"), { status: 415 });
    ...
    session.selectedId = selectCurrent(session)?.id || '';
    if (!session.selectedId) throw new Error('No Squished image was produced');
...
async function startSession(hash, rawOptions) {
  ...
  if (!candidate) throw Object.assign(new Error('Squishing images currently requires a local copy'), { status: 404 });
  ...
  if (!RASTER_EXTENSIONS.has(extension) || !String(candidate.mime || '').startsWith('image/')) {
    throw Object.assign(new Error('This image format is not supported for Squish yet'), { status: 415 });
  }
...
function sessionFor(id) {
  ...
  if (!session) throw Object.assign(new Error('Squish preview expired'), { status: 404 });
  session.updatedAt = Date.now();
  return session;
}
...
async function serveFile(req, res, url) {
  ...
  if (kind === 'original') {
    ...
    if (!candidate) throw Object.assign(new Error('Squished preview is not ready'), { status: 404 });
    path = candidate.path;
    mime = candidate.mime;
...
async function commitSession(session, candidateId, mode) {
  if (session.status !== 'ready') throw Object.assign(new Error('Squish preview is not ready'), { status: 409 });
  if (!['keep', 'replace'].includes(mode)) throw Object.assign(new Error('Choose Save squished or replace original'), { status: 400 });
  const candidate = session.candidates.find(item => item.id === candidateId) || session.candidates.find(item => item.id === session.selectedId);
  if (!candidate) throw Object.assign(new Error('Choose a Squished preview'), { status: 400 });
  ...
  if (!candidateInfo?.isFile() || Number(candidateInfo.size) !== Number(candidate.size)) throw new Error('Squished file failed verification');
  ...
  if (await sha256(stage) !== candidate.hash) {
    await unlink(stage).catch(() => {});
    throw new Error('Squished copy failed verification');
  }
