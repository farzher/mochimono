...
async function encodeSession(session) {
  try {
    ...
    if (requested.format === 'webp') {
      ...
      specs.push({ format: 'avif', quality: Math.max(50, requested.quality - 2), lossless: false });
    }
...
async function commitSession(session, candidateId, mode) {
  ...
  if (!previewCandidate) throw Object.assign(new Error('Choose a compressed preview'), { status: 400 });
