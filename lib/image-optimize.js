...
function normalizeOptions(raw = {}) {
  ...
  const resizePercent = [25, 50, 75, 100].includes(requestedPercent) ? requestedPercent : 0;
...
async function encodeSession(session) {
  try {
    ...
    for (let index = 0; index < specs.length; index++) {
      ...
      // Auto is conservative: if lossless already removes at least a quarter
      // of the bytes, stop instead of creating an unnecessary lossy generation.
...
function beginEncoding(session) {
...
function drainQueue() {
  while (activeEncodes < MAX_ACTIVE && encodeQueue.length) {
    const session = encodeQueue.shift();
    if (!session || session.status !== 'queued' || !sessions.has(session.id)) continue;
    beginEncoding(session);
  }
}
...
async function startSession(hash, rawOptions) {
  ...
  if (activeEncodes < MAX_ACTIVE) beginEncoding(session);
  else encodeQueue.push(session);
  return session;
}
