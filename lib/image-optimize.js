...
function outputPipeline(session) {
  let image = metadataPipeline(session.source.path);
  if (session.target.width !== session.metadata.width || session.target.height !== session.metadata.height) {
    image = image.resize(session.target.width, session.target.height, {
      fit: 'fill',
...
async function encodeCandidate(session, spec, { effort = session.options.effort, id = '', record = true } = {}) {
  ...
  let image = outputPipeline(session);
  if (spec.format === 'avif') {
    image = image.avif({
      quality: spec.quality,
      effort: effort === 'max' ? 9 : 4,
      chromaSubsampling: '4:4:4'
    });
  } else {
    image = image.webp(spec.lossless ? {
      lossless: true,
      effort: effort === 'max' ? 6 : 4
    } : {
      quality: spec.quality,
      effort: effort === 'max' ? 6 : 4,
      smartSubsample: true
  ...
  const [info, metadata, hash] = await Promise.all([
    stat(path),
    sharp(path, { limitInputPixels: false }).metadata(),
    sha256(path)
  ]);
  if (!info.isFile() || !info.size) throw new Error('Encoder produced an empty file');
  if (Number(metadata.width) !== Number(session.target.width) || Number(metadata.height) !== Number(session.target.height)) {
...
async function encodeSession(session) {
  try {
    ...
    session.target = targetDimensions(session.metadata.width, session.metadata.height, session.options.resizeMax);
    ...
    for (let index = 0; index < specs.length; index++) {
      ...
      // Auto remains conservative: if a lossless-codec result already removes
      // at least a quarter of the bytes, avoid an unnecessary lossy generation.
...
async function maxEffortCandidate(session, candidate) {
  if (candidate.effort === 'max') return candidate;
  if (activeEncodes >= MAX_ACTIVE) throw Object.assign(new Error('Another image encode is still running'), { status: 409 });
  activeEncodes++;
  try {
    const spec = {
    ...
    const final = await encodeCandidate(session, spec, {
      effort: 'max',
      id: `final-${randomUUID()}`,
      record: false
    });
    session.finalPaths.push(final.path);
    return final;
  } finally {
    activeEncodes--;
  }
...
async function commitSession(session, candidateId, mode) {
  ...
  const previewCandidate = session.candidates.find(item => item.id === candidateId) || session.candidates.find(item => item.id === session.selectedId);
  if (!previewCandidate) throw Object.assign(new Error('Choose an optimized preview'), { status: 400 });
  ...
  // Preview uses normal effort so comparison is fast. Only after the user has
  // chosen to save do we spend the time on the permanent max-effort encode.
  const candidate = await maxEffortCandidate(session, previewCandidate);

  const currentAfterEncode = await stat(session.source.path).catch(() => null);
  if (!sameStamp(currentAfterEncode, session.source)) {
    throw Object.assign(new Error('The original changed while the final encode was running. Generate a new preview first.'), { status: 409 });
  }
  ...
  session.result = {
    ...
    size: candidate.size,
    previewSize: previewCandidate.size,
    hash: candidate.hash,
