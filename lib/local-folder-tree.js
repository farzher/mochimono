...
function subtreeRows(db, root, prefix) {
  ...
  const fileRows = db.prepare(`
    SELECT path,size,mtime_ms AS mtimeMs,${catalogHashSql} AS hash
    FROM files
    WHERE root = ? ${range}
      AND instr(substr(path, ?), '/') = 0
    ORDER BY path
...
function addIndexedNode(db, configured, wantedParts, folders, files) {
  ...
  if (wantedParts.length < root.length && startsWithParts(root, wantedParts)) {
    const references = Number(db.prepare('SELECT COUNT(*) AS count FROM files WHERE root = ?').get(configured.root)?.count) || 0;
  ...
  for (const row of folderRows) {
    ...
    addFolder(folders, name, keyFor([...wantedParts, name]), Number(row.refCount) || 0);
  }
