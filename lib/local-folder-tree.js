...
function treeVersion(folders) {
  return [
    fileStamp(SYNC_INDEX_PATH),
    fileStamp(`${SYNC_INDEX_PATH}-wal`),
    ...folders.map(folder => `${folder.protected ? 'p' : 'b'}:${folder.root}:${folder.path}`)
  ].join('|');
}
...
function buildTree(folders) {
  ...
  if (!folders.length || !existsSync(SYNC_INDEX_PATH)) return nodes;
  ...
  for (const value of nodes.values()) {
    value.files.sort((a, b) => a.filename.localeCompare(b.filename, undefined, { numeric:true, sensitivity:'base' }));
  }
  return nodes;
}
...
function freshTree() {
  ...
  const version = treeVersion(folders);
  if (cachedNodes && cachedVersion === version) return cachedNodes;
  cachedNodes = buildTree(folders);
...
export function localFolderTree(path = '') {
  ...
  const current = freshTree().get(wanted);
