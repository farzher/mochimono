...
function watchIndex() {
  ...
  index.close();
  try {
    watcher = watch(SYNC_INDEX_PATH, () => schedule(180));
    watcher.unref?.();
    watcher.on('error', () => {
      watcher?.close();
      watcher = null;
  ...
  try {
    for (const root of settings.browseFolders) {
      ...
      for (const [path, state] of states) {
        ...
        if (!file?.id || !size || size > MAX_FAST_FILE_BYTES || !media(path)) continue;
        pending.push({
          root,
          key,
          path,
...
async function run() {
  ...
  try {
    watchIndex();
    if (settings.thumbnailMode === 'off') return schedule(5000);
    ...
    const learned = await learn(rows);
    schedule(learned ? ACTIVE_RETRY_MS : 1000);
  } catch (error) {
