...
async function syncRenditions() {
  ...
  for (const row of rows) {
    if (synced.get(row.original_hash)?.rendition_hash === row.rendition_hash) continue;
...
async function syncOnce(forcePresence = false) {
  ...
  try {
    ...
    if (!/not connected|offline|fetch failed|ECONNREFUSED/i.test(String(error?.message || ''))) console.warn('Compression metadata sync failed', error.message || error);
  } finally {
    running = false;
  }
}
