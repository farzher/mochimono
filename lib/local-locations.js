...
function localCacheStats() {
  if (cachedLocalStats && Date.now() - cacheStatsAt < CACHE_STATS_TTL) return cachedLocalStats;

  let bytes = 0;
  const stack = [CONFIG_DIR];
  const visited = new Set();
  while (stack.length) {
    const directory = stack.pop();
    let key;
    try { key = resolve(directory); } catch { continue; }
    ...
    for (const entry of entries) {
      ...
      try { bytes += Number(statSync(path).size) || 0; } catch {}
    }
  }
  ...
  if (existsSync(SYNC_INDEX_PATH)) {
    ...
    try {
      ...
      indexedFiles = Number(db.prepare('SELECT COUNT(*) AS count FROM file_hashes').get()?.count) || 0;
    } catch {
    } finally {
      try { db?.close(); } catch {}
    }
  ...
  cachedLocalStats = { path:CONFIG_DIR, bytes, indexedFiles, capacityBytes, freeBytes };
  return cachedLocalStats;
}
