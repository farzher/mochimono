...
function localFolders() {
  const protectedFolders = (settings.folders || [])
    ...
    .filter(Boolean)
    .map(path => {
      const configured=(settings.folders||[]).find(folder=>pathKey(configuredPath(folder))===pathKey(path));
      return { path, rootKey:pathKey(path), protected:true, importId:Number(configured?.importId)||0 };
    });
    ...
    try {
      if (hash) {
        rows.push(...db.prepare(`
          SELECT root,path,content_hash AS hash
          FROM files
          WHERE content_hash = ?
        `).all(String(hash)));
      } else {
  ...
  try {
    for (let offset = 0; offset < unresolved.length; offset += 400) {
      ...
      const rows = db.prepare(`
        SELECT root,path,size,mtime_ms AS mtimeMs,content_hash AS hash
        FROM files
        WHERE content_hash IN (${placeholders})
        ORDER BY content_hash,root,path
      `).all(...chunk);
...
export function localFolderPreview(path, limit = 5) {
  ...
  if (existsSync(SYNC_INDEX_PATH)) {
    ...
    try {
      ...
      rows.push(...db.prepare(`
        SELECT root,path,size,mtime_ms AS mtimeMs,content_hash AS contentHash,${catalogHashSql} AS hash
        FROM files
...
export function localCatalog(limit = 720, path = '', offset = null) {
  if (String(path || '') === CACHE_STATS_PATH) {
    return { files:[], folderSamples:[], nextOffset:null, cacheStats:localCacheStats() };
  }
    ...
    try {
      ...
      if (roots.length) {
        ...
        const fetched = db.prepare(`
          SELECT root,path,size,mtime_ms AS mtimeMs,${catalogHashSql} AS hash
          FROM files
