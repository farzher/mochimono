...
export function localCatalog(limit = 720) {
  const folders = localFolders();
  const safeLimit = Math.max(1, Math.min(3000, Number(limit) || 720));
  if (!folders.length) return { files: [], folderSamples: [] };
  ...
  // The Library uses a larger fast-catalog request and does not need Storage's
  // folder mosaic samples. Avoid those extra per-folder reads on the critical
  // first-paint path; the Storage dashboard's 720-row request still gets them.
  return { files, folderSamples: safeLimit <= 720 ? folderSamples(folders) : [] };
}
