...
export function localCatalog(limit = 720, path = '', offset = null) {
  ...
  const catalogFolders = path ? folders : folders.filter(folder => folder.available !== false);
  ...
  if (!catalogFolders.length) return { files: [], folderSamples: !paged && !path ? folderSamples(folders) : [], nextOffset: null };

  const byRoot = new Map(catalogFolders.map(folder => [folder.rootKey, folder]));
