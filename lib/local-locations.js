...
export function localCatalog(limit = 720) {
  const rawLimit = String(limit ?? '720');
  const includeSamples = rawLimit.endsWith(':samples');
  const requestedLimit = Number(rawLimit.replace(/:samples$/, ''));
  const folders = localFolders();
  const safeLimit = Math.max(1, Math.min(2000, requestedLimit || 720));
  if (!folders.length) return includeSamples ? { files: [], folderSamples: [] } : { files: [] };
  ...
  return includeSamples
    ? { files, folderSamples: folders.map(folder => localFolderPreview(folder.rootPath, 5)) }
    : { files };
}
