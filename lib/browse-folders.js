...
async function startParallelBrowseJob(root, label, work, { background = true, type = 'sync' } = {}) {
  ...
  setImmediate(() => browseJobScope.run(job, async () => {
    try {
      ...
      if (type === 'hash' && browseFolderFor(root)) scheduleBrowseHash(root, 500);
    }
  }));
  return job;
}
...
async function runPreviewWarm() {
  ...
  const wait = state.pauseUntil > Date.now() ? state.pauseUntil - now
...
function scheduleBrowseHash(path, delay = 250) {
  ...
  const timer = setTimeout(async () => {
    ...
    if (!pending) {
      await compactKnownBrowseHashes(root).catch(() => {});
      return;
    }
...
async function waitForRootWorkToStop(root) {
  ...
  const parallel = parallelBrowseJobs.get(key);
  if (parallel?.status === 'running') parallel.cancelRequested = true;
  for (let attempt = 0; attempt < 200 && parallelBrowseJobs.has(key); attempt++) {
    await new Promise(resolvePromise => setTimeout(resolvePromise, 25));
  }
  const running = currentJob();
  if (running?.status === 'running' && running.progress?.path && pathKey(running.progress.path) === key) {
    for (let attempt = 0; attempt < 200 && currentJob()?.status === 'running'; attempt++) {
      await new Promise(resolvePromise => setTimeout(resolvePromise, 25));
    }
  }
}
...
async function finalizeBrowseHashes(root) {
  clearTimeout(hashTimers.get(pathKey(root)));
  hashTimers.delete(pathKey(root));
