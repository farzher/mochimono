...
async function checkPreviewCache(state) {
  ...
  for (const row of rows) {
    ...
    if (providerThumbnailQueueStatus().background >= queueTarget) {
      // Stop scanning while the generation queue is full. The previous loop
      // repeatedly re-read 512 cache rows while waiting for one queue slot,
      // which looked stuck and created pointless disk/CPU work.
...
async function verifyPreviewCache(state) {
  ...
  // Something vanished, failed transiently, or was not actually queued. Re-run
  // the direct cache pass; completed entries remain hot in provider-thumbs.
  state.queued = 0;
...
async function runPreviewWarm() {
  ...
  if (settings.thumbnailMode !== 'max' && currentJob()?.status === 'running') {
    schedulePreviewWarm(1000);
    return;
  }
  ...
  try {
    ...
    console.warn(`Mochimono thumbnail check paused for ${state.path}: ${error?.message || error}`);
  }
...
export async function indexBrowseFolder(path, update = () => {}) {
  ...
  function queuePreview(filePath, file, hash) {
    ...
    }, { background: Boolean(currentJob()?.background) });
    return true;
  }
...
function queue(path, delay = 800, foreground = false) {
  ...
  const timer = setTimeout(() => {
    ...
    if (currentJob()?.status === 'running') return queue(root, 1500, isForeground);
...
async function updateBrowseChanges(root, names, update = () => {}) {
  ...
  try {
    ...
    for (const name of names) {
      ...
      if (file.isDirectory()) {
        // Directory structure events are ambiguous across platforms. They remain
        // the rare correctness fallback to a full check; ordinary child-file
        // events and large deletion bursts stay incremental.
        needsFull = true;
        break;
      }
...
function flushChanges(root, delay = CHANGE_DELAY_MS) {
  ...
  const timer = setTimeout(() => {
    ...
    const names = [...pending];
    pendingChanges.delete(key);
    const started = beginJob('sync', `Update ${basename(root) || root}`, update => incrementalIndexed(root, names, update), { background: true });
    if (!started) {
...
export async function browseFolderStats() {
  ...
  try {
    return await Promise.all(settings.browseFolders.map(async path => {
      ...
      const queued = timers.has(localKey) || changeTimers.has(localKey) || Boolean(pendingChanges.get(localKey)?.size);
      const runningHere = running?.status === 'running' && running?.progress?.path && pathKey(running.progress.path) === localKey;
      const automatic = (queued && !foregroundChecks.has(localKey)) || (runningHere && running.background);
      return {
        path,
