...
const PREVIEW_OWNER_QUEUE_MIN = 16;
...
function previewOwnerQueueTarget() {
  const unfinished = Math.max(1, [...previewWarmers.values()].filter(state => !state.done).length);
  return Math.max(PREVIEW_OWNER_QUEUE_MIN, Math.floor(PREVIEW_QUEUE_TARGET / unfinished));
}
