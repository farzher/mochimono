...
async function checkPreviewCache(state) {
  ...
  for (const row of rows) {
    ...
    if (providerThumbnailQueueStatus().background >= queueTarget) {
      // Revisit this exact row after the queue drains enough; do not silently
      // advance past missing work.
...
async function runPreviewWarm() {
  ...
  try {
    if (!existsSync(state.path)) {
      ...
      if (!queue.background && !queue.backgroundActive) resetFullPass(state, 'verifying');
