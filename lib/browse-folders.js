...
async function runPreviewWarm() {
  ...
  // Idle remains polite. Max is allowed to keep thumbnail work moving alongside
  // another Agent operation; provider-thumbs has its own bounded worker pool.
...
function queue(path, delay = 800) {
  ...
  const timer = setTimeout(() => {
    ...
    beginJob('sync', `Check ${basename(root) || root}`, update => indexed(root, update));
  }, delay);
  timer.unref?.();
...
function flushChanges(root, delay = CHANGE_DELAY_MS) {
  ...
  const timer = setTimeout(() => {
    ...
    const names = [...pending];
    pendingChanges.delete(key);
    const started = beginJob('sync', `Update ${basename(root) || root}`, update => incrementalIndexed(root, names, update));
    if (!started) {
...
export function startBrowseService(changeHandler = () => {}) {
  ...
  for (const path of settings.browseFolders) {
    ...
    // full-folder reconciliation anymore.
