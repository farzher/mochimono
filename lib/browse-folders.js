...
async function checkPreviewMedia(state) {
  ...
  // The indexed set can shrink or grow while thumbnails are warming. A complete
  // verification pass is the authoritative current total, so do not keep an old
  // count from before a directory change and loop the warmer from zero.
...
export async function indexBrowseFolder(path, update = () => {}) {
  ...
  if (backgroundPreviewsEnabled()) {
    if (!previousIndexedAt) resetPreviewWarm(root, indexedAt);
      ...
      // A metadata reconciliation that did not discover changed media must not
      // throw away an in-flight thumbnail pass. Rebase it onto the new index
      // generation and let its final verification pass reconcile the exact set.
...
function queueChange(path, filename) {
  ...
  // A large watcher burst is still incremental work. Previously crossing this
  // threshold converted the whole thing into a full-root rescan, which made a
  // large subfolder deletion restart thumbnail discovery from zero.
