...
async function encodeImage(job, candidate, options, sourceInfo) {
  ...
  if (metadata.depth && metadata.depth !== 'uchar') throw new Error('16-bit/HDR image compression is not supported safely yet');
  if (String(metadata.space || '').toLowerCase() === 'cmyk') throw new Error('CMYK image compression is not supported safely yet');
  ...
  if (!chosen) throw new Error('No compressed image was produced');
...
async function encodeVideo(job, candidate, options, sourceInfo) {
  ...
  if (!info.isFile() || !info.size) throw new Error('FFmpeg did not produce a compact video');
