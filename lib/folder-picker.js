...
public static class MochimonoFolderPicker {
  ...
  public static string[] Pick(string title, bool multiple) {
    ...
    try {
      ...
      // Use the standard Common Item Dialog option set used by native file-dialog
      // implementations for multi-folder selection. The previous NOVALIDATE /
      // NOTESTFILECREATE additions made Explorer treat its quoted multi-selection
      // text as a literal folder name on some Windows versions.
      uint options;
      dialog.GetOptions(out options);
      options |= FOS_PICKFOLDERS | FOS_FORCEFILESYSTEM;
      if (multiple) options |= FOS_ALLOWMULTISELECT;
      else options &= ~FOS_ALLOWMULTISELECT;
