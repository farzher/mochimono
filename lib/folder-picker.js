...
public static class MochimonoFolderPicker {
  ...
  public static string[] Pick(string title, bool multiple) {
    ...
    try {
      ...
      // Preserve Windows' native Open-dialog defaults, then add folder mode.
      // NOVALIDATE + NOTESTFILECREATE are important for multi-folder mode:
      // Explorer mirrors a Ctrl-selected set into the filename box as quoted
      // names ("2013" "2014"). Without these flags it can validate that text
      // as one folder name and reject an otherwise valid selection.
      uint options;
