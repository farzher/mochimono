...
public static class MochimonoFolderPicker {
  ...
  public static string[] Pick(string title, bool multiple) {
    ...
    try {
      ...
      // Do not inherit the FileOpenDialog defaults here. In folder mode some
      // combinations make Explorer treat a multi-selection like a quoted file
      // name ("Folder A" "Folder B") and reject it as an invalid folder name.
      // This mirrors the flags used by modern .NET FolderBrowserDialog.
      uint options = FOS_PICKFOLDERS | FOS_FORCEFILESYSTEM | FOS_FILEMUSTEXIST;
